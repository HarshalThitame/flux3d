...
const materialHighlights = [
  ...
  {
    label: "ABS / ASA",
    value: "Heat-ready parts",
    meta: "Built for demanding jobs",
  },
...
export default function MaterialsHero() {
  ...
  return (
    <section
      ref={ref}
      className="materials-hero-premium relative isolate overflow-hidden px-4 pb-12 pt-8 text-[#070b1d] md:px-8 lg:px-16"
    >
      <MaterialsPremiumFX />
      ...
      >
        <motion.div
          variants={item}
          className="mb-4 flex items-center gap-2 text-sm font-medium text-[#4B5563]"
        >
          <Link href="/" className="transition hover:text-[#070b1d]">
            Home
          </Link>
        ...
        <div className="grid min-w-0 gap-10 lg:grid-cols-[minmax(0,1fr)_430px] lg:items-start">
          <div className="min-w-0">
            ...
            >
              Compare finish, strength, heat resistance, flexibility, and cost
              before you upload. Flux3D pairs each job with a material that fits
              the part, not just the printer.
            </motion.p>
            ...
            >
              {heroStats.map((stat) => (
                <div
                  key={stat.label}
                  className="min-w-0 rounded-lg border border-purple-200 bg-white p-4 shadow-sm"
                >
                  <stat.icon className="mb-4 h-4 w-4 text-[#6d28d9]" />
                  <div className="text-xs font-bold uppercase text-[#4B5563]">
                    {stat.label}
                  </div>
                  <div className="mt-1 text-sm font-extrabold text-[#070b1d]">
                    {stat.value}
                  </div>
                </div>
              ))}
          ...
          >
            ...
            <div className="materials-shortlist">
              {materialHighlights.map((material) => (
                ...
                >
                  <div className="min-w-0">
                    <div className="text-sm font-extrabold text-[#070b1d]">
                      {material.label}
                    </div>
            ...
            <div className="materials-selection-rule">
              <p className="text-xs font-semibold uppercase text-[#4B5563]">
                Selection rule
              </p>
              <p className="mt-2 text-sm leading-6 text-[#374151]">
                Strong parts start with PETG, ABS, ASA, or Nylon. Display pieces
                start with PLA+, Silk PLA, or Resin.
              </p>
            </div>
          </motion.div>
        </div>
