...
export async function createCartOrderAction(
  ...
  for (const item of input.items) {
    ...
    if (!item.material.trim() || !item.color.trim()) {
      throw new Error(
        `Item "${item.fileName}" is missing material or color selection.`,
      );
    }
  }
  ...
  if (clearDefaultAddressError) {
    console.error(
      "[orders] Failed to clear default delivery addresses:",
      clearDefaultAddressError,
    );
  }
  ...
  if (existingAddress) {
    ...
    if (addressUpdateError) {
      console.error(
        "[orders] Failed to update delivery address:",
        addressUpdateError,
      );
    }
  } else {
    ...
    if (addressInsertError) {
      console.error(
        "[orders] Failed to insert delivery address:",
        addressInsertError,
      );
    }
  }
  ...
  for (const item of input.items) {
    ...
    if (item.modelVolumeMm3 && item.modelVolumeMm3 > 0 && item.fileUrl) {
      ...
      const volumeCheck = await verifyModelVolume(
        safeFileUrl,
        item.modelVolumeMm3,
      );
      if (!volumeCheck.valid) {
        throw new Error(
          volumeCheck.error ??
            `Volume verification failed for "${item.fileName}".`,
        );
      }
    }

    // Price locked at quote time — do not recalculate from raw inputs
    const itemMaterialCost = normalizeNumber(
      item.materialCost ?? 0,
      "material cost",
    );
    const itemMachineCost = normalizeNumber(
      item.machineCost ?? 0,
      "machine cost",
    );
    ...
    const overheadPercent = normalizeNumber(
      providedOverheadPercent > 0
        ? providedOverheadPercent
        : settings.overheadPercentage,
      "overhead percent",
    );
    const providedMarginPercent = Number(item.marginPercentage ?? 0);
    const marginPercent = normalizeNumber(
      providedMarginPercent > 0
        ? providedMarginPercent
  ...
  const totalPriceTotal = roundMoney(
    basePrices.reduce((sum, price) => sum + price, 0),
  );
  ...
  const cartWaterfall = calculatePricingWaterfall({
    ...
    quantity: preparedItems.reduce(
      (sum, item) => sum + item.normalizedQuantity,
      0,
    ),
    overheadPercent: 0,
    marginPercent: 0,
  ...
  if (couponId || couponCode) {
    let couponQuery = adminSupabase
      .from("coupons")
      .select(
        "id, code, discount_type, discount_value, max_discount, min_order_value, starts_at, expires_at, is_active, usage_limit, usage_per_user, used_count, first_order_only",
      );

    couponQuery = couponId
      ? couponQuery.eq("id", couponId)
      : couponQuery.eq("code", couponCode);
    const { data: coupon, error: couponError } =
      await couponQuery.maybeSingle();

    if (couponError) throw new Error(couponError.message);
    if (!coupon)
      throw new Error("Coupon no longer exists. Remove it and try again.");
    ...
    if (afterCartTotal < Number(coupon.min_order_value ?? 0)) {
      throw new Error(
        `Minimum order value of ₹${Number(coupon.min_order_value ?? 0).toLocaleString("en-IN")} required for this coupon.`,
      );
    }
  ...
  if (offerId) {
    const { data: offer, error: offerError } = await adminSupabase
      .from("offers")
      .select(
        "id, title, badge_text, sale_label, offer_type, discount_value, max_discount, starts_at, ends_at, is_active, usage_limit, usage_per_user, used_count",
      )
      .eq("id", offerId)
      .maybeSingle();
    ...
    const isOfferValid = Boolean(
      ...
      (offer.usage_limit == null ||
        Number(offer.used_count ?? 0) < Number(offer.usage_limit)),
    );
    ...
    if (isOfferValid && offer) {
      ...
      offerPromotion = offerFreeShipping
        ...
        : {
            ...
            maxDiscount:
              offer.max_discount == null ? null : Number(offer.max_discount),
          };
    } else {
      offerId = null;
  ...
  const afterAllDiscounts = basePrices.map((base, index) =>
    roundMoney(
      base -
        cartDiscountShares[index] -
        couponDiscountShares[index] -
        offerDiscountShares[index],
    ),
  );
  const deliveryShares = allocateAmount(
    resolvedDeliveryChargeTotal,
    afterAllDiscounts,
  );
  ...
  const orderItems = preparedItems.map((item, index) => {
    ...
    return {
      ...
      status: "pending",
      discount: roundMoney(
        cartDiscountForItem + couponDiscountForItem + offerDiscountForItem,
      ),
  ...
  if (!insertedOrders || insertedOrders.length === 0) {
    throw new Error(
      "Order submission did not return a confirmation. Please try again.",
    );
  }
  ...
  for (let i = 0; i < insertedOrders.length; i++) {
    ...
    if (order.serial_number == null || !order.created_at) {
      throw new Error(
        "Order submission returned incomplete confirmation details. Please try again.",
      );
    }
    ...
    const { error: modelFileError } = await adminSupabase
      .from("model_files")
      .upsert(
        {
          user_id: auth.user.id,
    ...
    if (modelFileError) {
      console.error(
        "[cart-orders] Failed to track model file:",
        modelFileError,
      );
    }

    const { data: quoteVersionRow, error: quoteVersionError } =
      await adminSupabase
        .from("quote_versions")
        .insert({
    ...
    if (quoteVersionError) {
      console.error(
        "[cart-orders] Failed to create quote version:",
        quoteVersionError,
      );
  ...
  void trackFeatureUsage(auth.user.id, "order_placed", {
    ...
    reportError(error, "Cart order feature tracking failed", {
      module: "tracking",
      level: "warn",
      tags: { flow: "cart_order", orderId: insertedOrders[0].id },
    }),
  );
  ...
  sendOrderPlacedCustomer(
    ...
    auth.profile.email,
    formatOrderNumber(
      insertedOrders[0].serial_number,
      insertedOrders[0].created_at,
    ),
    auth.profile.name,
    String(groupWaterfall.grandTotal ?? 0),
    orderItemsEmail,
    orderUrl,
  ).catch((error) =>
    reportError(error, "Cart order customer email failed", {
      module: "email",
      level: "warn",
      tags: { flow: "cart_order", orderId: insertedOrders[0].id },
    }),
  ...
  sendOrderPlacedAdmin(
    ...
    reportError(error, "Cart order admin email failed", {
      module: "email",
      level: "warn",
      tags: { flow: "cart_order", orderId: insertedOrders[0].id },
    }),
...
export async function prepareCartPaymentAction(
  ...
  const receipt = makeReceipt(
    capture.reference.replace(/[^a-zA-Z0-9_-]/g, "").slice(0, 10) || "CC",
    1,
  );
...
export async function verifyCartPaymentAndCreateOrder(params: {
  ...
  if (!isCaptured && !isAuthorized) {
    throw new Error(
      `Payment is not confirmed. Status: ${razorpayPayment.status}`,
    );
  }
  ...
  if (existingAddr) {
    ...
    await supabase
      .from("addresses")
      .insert({ user_id: auth.user.id, ...savedAddress });
  }

  // Build order items from cart data
  const groupId = crypto.randomUUID();
  const orderItems: Array<Record<string, unknown>> = [];
  const afterCartTotal =
    Number(pricingData.subtotal ?? 0) -
    Number(pricingData.cartDiscountAmount ?? 0);
  ...
  for (const item of itemsData) {
    ...
    const itemMachineCost = normalizeNumber(
      Number(item.machineCost ?? 0),
      "machine cost",
    );
    const postProcessingCharges = normalizeNumber(
      Number(item.postProcessingCharges ?? 0),
      "post processing charges",
    );
    const itemSubtotal = normalizeNumber(
    ...
    const finalPrice =
      totalPrice -
      cartDiscountForItem -
      couponDiscountForItem -
      offerDiscountForItem +
      itemDelivery;
    ...
    orderItems.push({
      ...
      layer_height: normalizeNumber(
        Number(item.layerHeight ?? 0.2),
        "layer height",
      ),
      ...
      discount_type:
        pricingData.couponDiscountType ?? pricingData.offerDiscountType ?? null,
  ...
  for (let i = 0; i < insertedOrders.length; i++) {
    ...
    if (order.serial_number == null || !order.created_at)
      throw new Error("Order confirmation incomplete.");
  ...
  const firstOrder = insertedOrders[0];
  const firstOrderNumber = formatOrderNumber(
    firstOrder.serial_number,
    firstOrder.created_at,
  );
  ...
  if (offerId) {
    ...
    await adminSupabase.rpc("increment_offer_used_count", {
      offer_id: offerId,
    });
  }
  ...
  if (cartConvertError) {
    reportError(cartConvertError, "Failed to mark quote cart converted", {
      module: "cart",
      level: "warn",
    });
  }
  ...
  notifyPaymentCaptured({
    ...
    reportError(error, "Payment captured notification failed", {
      module: "email",
      level: "warn",
      tags: { flow: "cart_payment", orderId: firstOrder.id },
    }),
  );
  ...
  const orderItemsEmail = itemsData.map((item: Record<string, unknown>) => ({
    ...
    price: String(
      Number(item.grandTotal ?? item.finalPrice ?? item.price ?? 0),
    ),
  }));
  ...
  sendOrderPlacedCustomer(
    ...
    firstOrderNumber,
    auth.profile.name,
    String(capture.amountPaise / 100),
    orderItemsEmail,
    orderUrl,
  ).catch((error) =>
    reportError(error, "Cart payment customer email failed", {
      module: "email",
      level: "warn",
      tags: { flow: "cart_payment", orderId: firstOrder.id },
    }),
  ...
  sendOrderPlacedAdmin(
    ...
    reportError(error, "Cart payment admin email failed", {
      module: "email",
      level: "warn",
      tags: { flow: "cart_payment", orderId: firstOrder.id },
    }),
  ...
  notifyWhatsAppOrderConfirmed({
    ...
    reportError(error, "Cart WhatsApp confirmation failed", {
      module: "whatsapp",
      level: "warn",
      tags: { flow: "cart_payment", orderId: firstOrder.id },
    }),
  ...
  void trackFeatureUsage(auth.user.id, "order_placed", {
    ...
    unitCount: itemsData.reduce(
      (sum, item) => sum + Math.max(1, Math.floor(Number(item.quantity ?? 1))),
      0,
    ),
    couponCode: pricingData.couponCode as string | null,
    offerId,
    grandTotal: capture.amountPaise / 100,
  }).catch((error) =>
    reportError(error, "Cart payment feature tracking failed", {
      module: "tracking",
      level: "warn",
      tags: { flow: "cart_payment", orderId: firstOrder.id },
    }),
