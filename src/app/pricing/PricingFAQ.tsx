import { pricingFaqs } from '@/lib/pricing-content'
import Reveal from '@/components/Reveal'

export default function PricingFAQ({ whatsappNumber }: { whatsappNumber: string }) {
  return (
    <section className="lux-band-white relative px-6 py-20 md:py-28">
      <div className="mx-auto max-w-[860px]">
        <Reveal>
          <div className="mb-12 text-center">
            <p className="lux-eyebrow justify-center">Pricing FAQ</p>
            <h2 className="lux-heading-2 mt-4">How 3D printing costs work.</h2>
          </div>
        </Reveal>
        ...
        <div className="overflow-hidden rounded-3xl border border-[var(--lux-border-light,#E7E5E0)] bg-[var(--lux-bg-elevated,#FFFFFF)] shadow-[var(--lux-shadow-sm)]">
          {pricingFaqs.map((faq, index) => (
            <details
              key={faq.question}
              open={index === 0}
              className="group border-b border-[var(--lux-border-light,#E7E5E0)] last:border-b-0"
            >
              <summary className="flex cursor-pointer list-none items-center justify-between gap-4 px-6 py-5 transition-colors hover:bg-[var(--lux-bg-base,#FDFCF8)] [&::-webkit-details-marker]:hidden">
        ...
        <Reveal delay={140}>
          <div className="mt-8 text-center">
            <a
              href={`https://wa.me/${whatsappNumber}?text=Hi%20Flux3D!%20I%20have%20a%20question%20about%203D%20printing%20pricing.`}
              target="_blank"
