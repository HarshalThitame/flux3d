...
export async function createOrderAction(
  ...
  if (!rateLimit.success) {
    throw new Error(
      "Too many quote submissions. Please wait a moment and try again.",
    );
  }
  ...
  if (!input.modelMetadata || !input.modelMetadata.volumeMm3) {
    throw new Error(
      "Model metadata is missing. Re-upload the model and try again.",
    );
  }

  const normalizedQuantity = Math.max(
    1,
    Math.floor(normalizeNumber(input.quantity, "quantity")),
  );
  ...
  const fileName =
    safeFileUrl.split("/").pop() || `${input.quoteId ?? "model"}.stl`;
  ...
  const { data: insertedQuoteVersion, error: quoteVersionError } =
    await adminSupabase
      .from("quote_versions")
      .insert({
  ...
  if (quoteVersionError) {
    console.error(
      "[orders] Failed to create quote version:",
      quoteVersionError,
    );
  ...
  void trackFeatureUsage(auth.user.id, "order_placed", {
    ...
    reportError(error, "Instant-quote feature tracking failed", {
      module: "tracking",
      level: "warn",
      tags: { flow: "instant_quote" },
    }),
  );
  ...
  sendOrderPlacedCustomer(
    ...
    orderNumber,
    auth.profile.name,
    String(Number(insertedOrder.grand_total ?? breakdown.grandTotal)),
    itemsEmail,
    orderUrl,
  ).catch((error) =>
    reportError(error, "Instant-quote customer email failed", {
      module: "email",
      level: "warn",
      tags: { flow: "instant_quote", orderId: insertedOrder.id },
    }),
  );
  sendOrderPlacedAdmin(
    "",
    ...
    reportError(error, "Instant-quote admin email failed", {
      module: "email",
      level: "warn",
      tags: { flow: "instant_quote", orderId: insertedOrder.id },
    }),
...
export async function prepareQuotePaymentAction(
  ...
  const grandTotal =
    input.grandTotal ?? input.finalPrice + input.deliveryCharge;
  if (grandTotal <= 0)
...
export async function verifyQuotePaymentAndCreateOrder(params: {
  ...
  const normalizedQuantity = Math.max(
    1,
    Math.floor(normalizeNumber(Number(configData.quantity || 1), "quantity")),
  );
  const trimmedAddress = {
  ...
  if (clearDefaultError)
    console.error("[orders] Failed to clear defaults:", clearDefaultError);
  ...
  const { data: insertedOrder, error: insertError } = await adminSupabase
    ...
    .insert({
      ...
      layer_height: normalizeNumber(
        Number(configData.layerHeight || 0),
        "layer height",
      ),
      quantity: normalizedQuantity,
      ...
      price_per_unit: roundMoney(
        Number(pricingData.totalPrice ?? 0) / normalizedQuantity,
      ),
    ...
    .select(
      "id, serial_number, created_at, material, color, infill, layer_height, quantity, post_processing_level, supports, full_name, phone, address_line1, address_line2, city, state, pincode, landmark, material_cost, machine_cost, subtotal, post_processing_charges, weight, difficulty_factor, overhead_percent, overhead_amount, margin_percent, margin_amount, total_price, cart_discount_percent, cart_discount, final_price, delivery_charge, grand_total, price, estimated_time, status, notes, created_at",
    )
    .single();

  if (insertError) {
    if (insertError.message.includes("quote_captures"))
  ...
  notifyPaymentCaptured({
    ...
    reportError(error, "Payment captured notification failed", {
      module: "email",
      level: "warn",
      tags: { flow: "quote_payment", orderId: insertedOrder.id },
    }),
  );
  ...
  sendOrderPlacedCustomer(
    ...
    orderNumber,
    auth.profile.name,
    String(Number(pricingData.grandTotal ?? 0)),
    itemsEmail,
    orderUrl,
  ).catch((error) =>
    reportError(error, "Quote customer email failed", {
      module: "email",
      level: "warn",
      tags: { flow: "quote_payment", orderId: insertedOrder.id },
    }),
  ...
  sendOrderPlacedAdmin(
    ...
    reportError(error, "Quote admin email failed", {
      module: "email",
      level: "warn",
      tags: { flow: "quote_payment", orderId: insertedOrder.id },
    }),
  ...
  notifyWhatsAppOrderConfirmed({
    ...
    reportError(error, "Quote WhatsApp confirmation failed", {
      module: "whatsapp",
      level: "warn",
      tags: { flow: "quote_payment", orderId: insertedOrder.id },
    }),
  ...
  const fileName =
    (draftData.fileUrl as string)?.split("/").pop() || "model.stl";
