...
export default async function InstantQuotePage({
  searchParams,
}: InstantQuotePageProps) {
  const params = await searchParams;
  const modelFileId =
    typeof params.modelFile === "string" ? params.modelFile : undefined;
  const auth = await getCurrentUserProfile();
  let initialModelFile:
    { fileName: string; fileUrl: string; material?: string | null } | undefined;
  ...
  if (auth && modelFileId) {
    ...
    if (data?.file_url) {
      initialModelFile = {
        fileName:
          data.file_name || data.file_url.split("/").pop() || "Uploaded model",
