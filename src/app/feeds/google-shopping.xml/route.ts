...
function makeFeedItem(product: ShopPublicProduct) {
  ...
  const lines = [
    ...
    `      <g:description>${escapeXml(
      truncate(
        cleanText(product.meta_description) ||
          ...
          extractTextFromBlocks(product.long_description_blocks) ||
          product.name,
    ...
    image
      ? `      <g:image_link>${escapeXml(new URL(image, siteUrl).toString())}</g:image_link>`
      : null,
    ...additionalImages.map(
      (url) =>
        `      <g:additional_image_link>${escapeXml(new URL(url, siteUrl).toString())}</g:additional_image_link>`,
    ),
  ...
  try {
    ...
    items = products
      .filter(
        (product) =>
          product.slug &&
          (product.thumbnail_url || product.image_urls.length > 0),
      )
      .map(makeFeedItem);
  } catch (error) {
