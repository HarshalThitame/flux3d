  ...
  return (
    <section ref={ref} className="relative py-24 px-6 overflow-hidden">
      ...
      <div className="max-w-[1200px] mx-auto relative z-10">
        ...
        >
          <p className="text-sm font-medium text-[#FF5C1A] uppercase tracking-[3px] mb-4">Why Trust Us</p>
          <h2 className="font-[var(--font-syne)] text-[clamp(1.8rem,4vw,3rem)] font-extrabold text-white tracking-[-1px] leading-[1.1]">
            Quality You Can{' '}
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#10B981] to-[#FF5C1A]">
              See and Touch.
            </span>
          </h2>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {trustPoints.map((point, i) => (
            <motion.div
              ...
              className="group bg-[#0d1120] border border-[rgba(255,255,255,0.07)] rounded-2xl p-6 hover:border-[rgba(255,92,26,0.2)] transition-colors"
            >
              <div className="w-12 h-12 rounded-xl bg-[rgba(255,92,26,0.08)] flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
                <point.icon className="w-6 h-6 text-[#FF5C1A]" />
              </div>
              <h3 className="font-[var(--font-syne)] text-base font-bold text-white mb-2 group-hover:text-[#FF5C1A] transition-colors">
                {point.title}
              </h3>
              <p className="text-sm text-[#7a82a0] leading-[1.6]">{point.body}</p>
            </motion.div>
          ))}
        </div>
      </div>
