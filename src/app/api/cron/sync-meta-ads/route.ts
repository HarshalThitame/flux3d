...
export async function GET(request: Request) {
  ...
  try {
    ...
    for (const local of localRecords ?? []) {
      ...
      const metaBudgetPaise = meta.daily_budget ? Number(meta.daily_budget) : null
      if (metaBudgetPaise !== null && local.daily_budget_paise !== metaBudgetPaise) {
        updates.daily_budget_paise = metaBudgetPaise
      }

      if (Object.keys(updates).length > 0) {
        updates.updated_at = new Date().toISOString()
        await supabase.from('meta_ad_campaigns').update(updates).eq('id', local.id)
        updated++
      }
    }
      ...
      for (const insight of insights) {
        ...
        if (budget > 0 && spend > budget * 1.5) {
          logWarn('Spend anomaly detected — auto-pausing campaign', {
            module: 'meta-ads',
            metadata: { campaignId: insight.campaign_id, spend, budget, multiplier: spend / budget },
          })
          try {
            ...
            logError('Failed to auto-pause anomalous campaign', {
              module: 'meta-ads',
              error: pauseErr instanceof Error ? pauseErr : new Error(String(pauseErr)),
            })
          }
        }
      }
    ...
    logInfo('Meta ads sync completed', {
      module: 'meta-ads',
      duration,
      metadata: { updated, archived: orphanedLocal, orphanedMeta, spendAnomalies },
    })
