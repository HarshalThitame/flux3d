...
async function checkOpenAI(): Promise<{ ok: boolean; error: string | null }> {
  ...
  try {
    ...
    await openai.models.retrieve(
      process.env.WHATSAPP_EMBEDDING_MODEL || "text-embedding-3-small",
    );
...
async function checkWhatsAppApi(): Promise<{
  ok: boolean;
  error: string | null;
}> {
...
async function checkRagMode(): Promise<{
  mode: "database" | "seed" | "empty";
  count: number;
}> {
  const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL;
  const serviceKey = process.env.SUPABASE_SERVICE_ROLE_KEY;
  ...
  try {
    ...
    return {
      mode: count && count > 0 ? "database" : "seed",
      count: count ?? 0,
    };
  } catch {
...
async function checkSupabaseRpc(): Promise<{
  ok: boolean;
  error: string | null;
}> {
  const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL;
...
export async function GET() {
  ...
  const isWhatsappPaused = process.env.WHATSAPP_ORDERING_ENABLED === "false";
  const allOk =
    db.ok && openai.ok && (isWhatsappPaused || whatsapp.ok) && rpc.ok;
