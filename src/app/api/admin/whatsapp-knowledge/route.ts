...
import {
  generateWhatsAppEmbedding,
  getWhatsappKnowledgeSeed,
  toKnowledgeChunk,
...
function normalizePayload(body: KnowledgePayload) {
  const sourceKey = normalizeWhatsAppKnowledgeSourceKey(
...
async function loadKnowledgeChunks() {
  ...
  const { data, error } = await supabase
    ...
    .select(
      "id, source_key, title, content, tags, priority, active, created_at, updated_at",
    )
