...
import { BulkToggleSchema } from '@/lib/admin/meta-ads-schemas'
...
/**
 * POST /api/admin/ads/bulk/toggle
 *
 * Toggles status for multiple campaigns in a single request.
 * Body: { ids: string[], status: 'ACTIVE' | 'PAUSED' | 'ARCHIVED' }
 */
export async function POST(request: Request) {
  try {
    const auth = await requireMetaAdsAuth(request, {
      rateLimit: { prefix: 'meta-ads-bulk-toggle', windowSeconds: 60, maxRequests: 10 },
    })
    ...
    const validation = validateBody(BulkToggleSchema, rawBody)
    ...
    const results = await Promise.allSettled(
      ids.map((id) => updateCampaignStatus(id, status))
    )
    ...
    logError('Meta bulk toggle error', {
      module: 'meta-ads',
      error: err instanceof Error ? err : new Error(String(err)),
    })
    return NextResponse.json(
      { error: err instanceof Error ? err.message : 'Failed to bulk toggle campaigns' },
      { status: 500 },
    )
  }
}
