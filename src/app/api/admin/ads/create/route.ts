...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const siteUrl = siteUrlRaw ?? (process.env.NEXT_PUBLIC_SITE_URL || 'https://flux3d.in')
    ...
    // ─── Idempotency check ───────────────────────────────────────────────
    const idempotencyKey = computeIdempotencyKey(user.id, params.categoryName, params.dailyBudgetPaise)
    ...
    return NextResponse.json({
      ...
      dpa: result.dpa
        ...
        : null,
      productsUsed: result.cards.map((c) => ({ name: c.name, price: c.description })),
    ...
    const message = err instanceof Error ? err.message : 'Failed to create Meta ad campaign'

    // Return structured errors from the service
    if (message.includes('No active category found')) {
      const categories = await listActiveCategories(await createServerClient()).catch(() => [])
      return NextResponse.json(
    ...
    if (message.includes('No active products found') || message.includes('No usable products')) {
      const categories = await listActiveCategories(await createServerClient()).catch(() => [])
      return NextResponse.json(
        { error: message, suggestion: 'Add products to this category or choose a different one.', availableCategories: categories },
        { status: 404 },
      )
    }
