...
export async function PUT(
  ...
  try {
    ...
    if ("event_name" in body)
      update.event_name = String(body.event_name).trim();
    if ("template_id" in body)
      update.template_id = String(body.template_id).trim();
    if ("target_audience" in body)
      update.target_audience = String(
        body.target_audience,
      ) as AutomationTargetAudience;
    if ("delay_minutes" in body)
      update.delay_minutes = Number(body.delay_minutes);
