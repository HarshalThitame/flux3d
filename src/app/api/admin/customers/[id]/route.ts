...
import {
  customerPatchSchema,
  zodErrorResponse,
} from "@/lib/admin/schemas/customers";
...
export async function GET(
  ...
  try {
    ...
    const { data: orders, error: ordersError } = await supabase
      .from("orders")
      .select(
        "id, order_number, material, weight, grand_total, final_price, total_price, status, created_at",
      )
      .eq("user_id", userId)
...
export async function PATCH(
  ...
  const requiresSuspendPermission =
    ...
    payload.manualCoupon !== undefined ||
    payload.manualCredit !== undefined;
  const permission = requiresSuspendPermission
    ? "customers.suspend"
    : "customers.update";
  ...
  try {
    ...
    if (payload.manualCoupon !== undefined)
      writes.manual_coupon = payload.manualCoupon;
    if (payload.manualCredit !== undefined)
      writes.manual_credit = payload.manualCredit;
