...
const PAGE_SIZE = 50
const REASONS = ['order_placed', 'order_cancelled', 'order_returned', 'reservation_expired', 'manual_adjust', 'restock', 'release', 'system']
...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (search) {
      query = query.or(`sku.sku_code.ilike.%${search}%,product.name.ilike.%${search}%`)
    }
