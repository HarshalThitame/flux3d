...
async function uploadSkuImage(supabase: ReturnType<typeof createAdminSupabaseClient>, file: File, productId: string, skuId: string) {
...
async function resolveProductId(supabase: ReturnType<typeof createAdminSupabaseClient>, skuId: string) {
  const { data, error } = await supabase.from('shelf_skus').select('product_id').eq('id', skuId).maybeSingle()
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (contentType.includes('application/json')) {
      ...
      if (images.length === 0) {
        return NextResponse.json({ error: 'At least one image entry is required.' }, { status: 400 })
      }
      const supabase = createAdminSupabaseClient()
      const { data, error } = await supabase
        .from('shelf_sku_images')
...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!body.image_id) return NextResponse.json({ error: 'image_id is required.' }, { status: 400 })

    const supabase = createAdminSupabaseClient()
    const updates: { alt_text?: string | null; display_order?: number; is_primary?: boolean } = {}
