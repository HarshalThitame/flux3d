...
export async function GET(request: Request) {
  ...
  try {
    ...
    const [variantsResult, skusResult] = await Promise.all([
      ids.length > 0
        ? supabase.from('shelf_variant_options').select('*').in('product_id', ids).order('display_order', { ascending: true })
        : Promise.resolve({ data: [] as unknown[], error: null }),
      ids.length > 0
        ? supabase.from('shelf_skus').select('*').in('product_id', ids).order('created_at', { ascending: true })
        : Promise.resolve({ data: [] as unknown[], error: null }),
    ])
    ...
    for (const variant of (variantsResult.data ?? []) as Array<{ product_id: string }>) {
      const list = variantsByProduct.get(variant.product_id) ?? []
      list.push(variant)
      variantsByProduct.set(variant.product_id, list)
    }
    const skusByProduct = new Map<string, unknown[]>()
    for (const sku of (skusResult.data ?? []) as Array<{ product_id: string }>) {
    ...
    return NextResponse.json(
      { products, exportedAt: new Date().toISOString(), count: products.length },
