  ...
  try {
    ...
    if (format === "csv") {
      ...
      records = list
        ...
        .filter((item): item is ImportProductRecord =>
      ...
      errors = list
        ...
        .map(({ index }) => ({
          row: index + 1,
          error: "Missing name or invalid row",
        }));
    }

    if (records.length > MAX_RECORDS) {
      return NextResponse.json(
        { error: `Too many products. Maximum is ${MAX_RECORDS} per import.` },
        { status: 400 },
      );
    }
    ...
    return NextResponse.json(
      { ...outcome, format },
      { status: outcome.imported > 0 ? 201 : 200 },
    );
  } catch (error) {
...
async function importProducts(
  ...
  const takenSlugs = new Set(
    (existingResult.data ?? []).map((row) => row.slug),
  );
  const categoryByName = new Map<string, string>();
  const categoryBySlug = new Map<string, string>();
  for (const category of (categoriesResult.data ?? []) as Array<{
    id: string;
    name: string;
    slug: string;
  }>) {
  ...
  for (let index = 0; index < records.length; index += 1) {
    ...
    try {
      ...
      const categoryId = record.category
        ? (categoryByName.get(record.category.trim().toLowerCase()) ??
          categoryBySlug.get(record.category.trim().toLowerCase()))
        : undefined;
      ...
      const { data: product, error: productError } = await supabase
        ...
        .insert({
          ...
          long_description_blocks: Array.isArray(record.long_description_blocks)
            ? record.long_description_blocks
            : [],
          category_id: categoryId ?? null,
          tags: Array.isArray(record.tags) ? record.tags : [],
          occasion_tags: Array.isArray(record.occasion_tags)
            ? record.occasion_tags
            : [],
          thumbnail_url: record.thumbnail_url ?? null,
          image_urls: Array.isArray(record.image_urls) ? record.image_urls : [],
          image_alt:
            record.image_alt && typeof record.image_alt === "object"
              ? record.image_alt
              : {},
          model_url: record.model_url ?? null,
          base_price: Number.isFinite(Number(record.base_price))
      ...
      const variants = (record.variants ?? [])
        .filter((variant): variant is ImportVariant =>
          Boolean(variant && typeof variant.option_name === "string"),
        )
        ...
        .map((variant, order) => ({
          ...
          values: Array.isArray(variant.values)
            ? variant.values.map(String)
            : [],
          display_order: Number.isFinite(Number(variant.display_order))
            ? Number(variant.display_order)
            : order,
      ...
      const skus = (record.skus ?? [])
        .filter((sku): sku is ImportSku =>
          Boolean(
            sku &&
            sku.variant_combination &&
            typeof sku.variant_combination === "object",
          ),
        )
        .map((sku, order) => ({
          product_id: product.id,
