...
async function uploadVariantImage(supabase: ReturnType<typeof createAdminSupabaseClient>, file: File, productId: string) {
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!optionName || !optionValue) {
      return NextResponse.json({ error: 'option_name and option_value are required.' }, { status: 400 })
    }
