...
export async function GET() {
  ...
  try {
    ...
    const data = (categories ?? []).map((category) => ({
      ...category,
      parent_name: category.parent_category_id ? names.get(category.parent_category_id) ?? null : null,
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    if ((count ?? 0) > 0) {
      return NextResponse.json({ error: 'Cannot delete a category with linked products.' }, { status: 409 })
    }
