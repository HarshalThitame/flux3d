    ...
    <main className="px-4 pb-20 pt-5 md:px-8 lg:px-16">
      <div className="mx-auto max-w-7xl">
        <div className="mb-8 flex flex-col gap-4 rounded-3xl border border-[var(--border-light)] bg-white p-6 shadow-[var(--shadow-sm)] md:flex-row md:items-end md:justify-between">
          ...
          <label className="flex min-h-[44px] items-center gap-3 rounded-xl border border-[var(--border-light)] bg-[var(--bg-soft)] px-4 text-sm font-semibold text-[var(--text-secondary)]">
            <input
              type="checkbox"
              checked={inStockOnly}
        ...
        {loading ? (
          ...
          <div className="rounded-3xl border border-rose-200 bg-rose-50 p-8 text-center font-semibold text-rose-700">
            {error}
          </div>
        ) : visibleProducts.length === 0 ? (
          <div className="rounded-3xl border border-[var(--border-light)] bg-white p-10 text-center shadow-[var(--shadow-sm)]">
            <Heart className="mx-auto h-14 w-14 text-rose-500" />
            <h2 className="mt-4 text-2xl font-extrabold text-[var(--text-primary)]">Nothing saved yet</h2>
          ...
          <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4">
            {visibleProducts.map((product) => (
              <ProductCard key={product.id} product={product} actionLabel="Move to Cart" />
            ))}
          </div>
        )}
      </div>
