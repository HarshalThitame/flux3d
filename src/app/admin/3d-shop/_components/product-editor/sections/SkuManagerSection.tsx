...
function SkuModelUpload({ sku }: { sku: ShopSku }) {
  ...
  const uploading = Object.entries(uploadState).some(
    ([key, state]) =>
      key.startsWith(uploadPrefix) && state.status === "uploading",
  );
  ...
  return (
    <label
      ...
      title={
        sku.model_url
          ? "Replace per-SKU 3D model"
          : "Upload a GLB override shown in the 3D viewer when this variant is selected"
      }
    >
      {uploading ? (
...
function SkuGallerySummary({ sku }: { sku: ShopSku }) {
  ...
  if (images.length === 0) {
    return (
      <span
        ...
        title="Assign images to this SKU in the Media section above"
      >
  ...
  return (
    <div
      ...
      title={`${images.length} image${images.length === 1 ? "" : "s"} — manage in the Media section above`}
    >
...
export function SkuManagerSection() {
  ...
  return (
    <div ref={skuSectionRef}>
      <Section
        title="SKU Manager"
        description="Edit generated variants, pricing, inventory, and per-variant media."
      >
        {/* Bulk toolbar */}
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="text-xs text-[#6F7192]">
              {validSelected.size > 0
                ...
                : "No selection — bulk actions apply to all"}
            </span>
            {validSelected.size > 0 && (
              <button
                type="button"
            ...
            <tbody>
              {skus.map((sku) => {
                ...
                return (
                  <tr
                    key={sku.id}
                    className={`border-b border-gray-100 last:border-0 ${isSelected ? "bg-[#6d28d9]/5" : ""}`}
                  >
                    ...
                    <td className="px-3 py-3">
                      <input
                        ...
                        onChange={(event) =>
                          updateSku(sku.id, "price", Number(event.target.value))
                        }
                        className={numInput}
                      />
                    </td>
        ...
        <div className="grid gap-3 md:hidden" data-sku-layout="cards">
          {skus.map((sku) => {
            ...
            return (
              <div
                key={sku.id}
                className={`rounded-2xl border bg-white p-4 ${isSelected ? "border-[#6d28d9]/40" : "border-gray-200"}`}
              >
                ...
                <div className="mt-3 flex items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    {isSelected && (
                      <span className="rounded-full bg-[#6d28d9]/10 px-2 py-0.5 text-xs font-semibold text-[#6d28d9]">
                        Selected
                      </span>
                    )}
                    <button
