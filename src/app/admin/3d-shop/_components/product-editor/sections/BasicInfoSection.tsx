...
import { AiAssistButton, AiGenerateAllButton, AiToneSelector } from '../AiAssist'
  ...
  return (
    <Section title="Basic Info" description="Core product details, copy, tags, and categorization.">
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <div>
          <label className="mb-1.5 block text-xs font-medium text-[#6F7192]" htmlFor="product-name">
            Name
          </label>
          <input
            id="product-name"
        ...
        <div>
          <label className="mb-1.5 block text-xs font-medium text-[#6F7192]" htmlFor="product-slug">
            Slug
          </label>
          <div className="flex items-center gap-2">
            <input
              ...
              onChange={(event) => {
                markSlugTouched()
                updateProduct('slug', event.target.value.replace(/[^a-z0-9]+/gi, '-').toLowerCase().replace(/^-+|-+$/g, ''))
              }}
              onBlur={() => markTouched('slug')}
              placeholder="auto-generated"
              className={`${inputClass} ${errors.slug ? 'border-rose-300 bg-rose-50/30' : ''}`}
            />
            {slugStatus === 'checking' && <Loader2 className="h-4 w-4 shrink-0 animate-spin text-[#6F7192]" />}
            {slugStatus === 'available' && <span className="shrink-0 text-xs font-semibold text-emerald-700">Available</span>}
            {slugStatus === 'taken' && <span className="shrink-0 text-xs font-semibold text-rose-600">Taken</span>}
          </div>
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <div>
          <label className="mb-1.5 block text-xs font-medium text-[#6F7192]" htmlFor="product-category">
            Category
          </label>
          <select
            id="product-category"
            value={product.category_id}
            onChange={(event) => updateProduct('category_id', event.target.value)}
        ...
        <div>
          <label className="mb-1.5 block text-xs font-medium text-[#6F7192]" htmlFor="product-price">
            Base Price (₹)
          </label>
          <input
            id="product-price"
            ...
            onChange={(event) => updateProduct('base_price', Number(event.target.value))}
      ...
      <div>
        <div className="mb-1.5 flex items-center justify-between">
          <label className="block text-xs font-medium text-[#6F7192]" htmlFor="product-description">
            Short Description
          </label>
          <AiAssistButton kind="short_description" compact label="AI" title="Generate short description with AI" />
        </div>
        <textarea
        ...
        <div className="mt-1 flex items-center justify-between">
          ...
          <span className={`ml-auto text-xs ${product.description.length > 200 ? 'font-semibold text-rose-600' : 'text-[#6F7192]'}`}>
      ...
      <div>
        <div className="mb-1.5 flex items-center justify-between">
          <span className="text-xs font-medium text-[#6F7192]">Long Description</span>
          <AiAssistButton kind="long_description" compact label="AI" title="Generate long description with AI" />
        </div>
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <TagInput
          ...
          action={<AiAssistButton kind="tags" compact label="AI" title="Suggest tags with AI" />}
        />
        <div>
          <div className="mb-2 flex items-center justify-between">
            <span className="text-xs font-medium text-[#6F7192]">Occasion Tags</span>
            <AiAssistButton kind="occasion_tags" compact label="AI" title="Suggest occasion tags with AI" />
          </div>
