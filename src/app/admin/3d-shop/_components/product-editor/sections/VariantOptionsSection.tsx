...
function ValueImageControl({
  ...
  const { variantOptionImages } = useProductEditor();
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      <span className="inline-flex items-center gap-1.5 rounded-xl border border-dashed border-gray-300 bg-gray-50/60 px-2.5 py-2 text-xs font-semibold text-[#6F7192]">
      ...
      {previews.map((image) => (
        <span
          key={image.id}
          className="relative h-9 w-9 overflow-hidden rounded-lg border border-gray-200"
          title={image.alt_text ?? `${optionName}: ${optionValue}`}
        >
          <Image
            src={image.image_url}
            alt={image.alt_text ?? `${optionName}: ${optionValue}`}
            fill
            sizes="36px"
      ...
      {images.length === 0 && (
        <span className="text-xs text-[#6F7192]">
          Assign images to this value in the Media section above.
        </span>
      )}
    </div>
  );
...
export function VariantOptionsSection() {
  ...
  return (
    <Section
      ...
      description="Define configurable choices that drive SKU generation."
    >
      {variants.length === 0 ? (
            ...
            return (
              ...
              >
                <div className="grid gap-3 lg:grid-cols-[auto_180px_170px_1fr_120px_80px_auto] lg:items-start">
                  ...
                  <label>
                    <span className="mb-1 block text-xs text-[#6F7192]">
                      Option Name
                    </span>
                    <select
                  ...
                  {["toggle", "text_input"].includes(variant.option_type) ? (
                    ...
                    <TagInput
                      ...
                      onChange={(values) =>
                        updateVariant(variant.id, "values", values)
                      }
                      placeholder="Type value and press Enter"
                    />
                  )}
                  {["toggle", "text_input"].includes(
                    variant.option_type,
                  ) ? null : (
