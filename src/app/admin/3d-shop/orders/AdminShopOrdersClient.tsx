...
export default function AdminShopOrdersClient() {
  ...
  const loadOrders = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/3d-shop/admin/orders?${params.toString()}`,
      );
  ...
  async function bulkUpdate(nextStatus: string) {
    ...
    try {
      ...
      setToast({
        type: "success",
        message: `Marked ${selectedIds.length} order${selectedIds.length === 1 ? "" : "s"} as ${nextStatus}.`,
      });
      await loadOrders();
    } catch (error) {
      setToast({
      ...
      >
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm text-[#6F7192]">
            Manage 3D Shop COD orders, status updates, and fulfilment details.
          </p>
        </div>
              ...
              <tr className="border-b border-gray-100">
                <th className="px-4 py-3 text-left">
                  <input
                    ...
                    checked={
                      orders.length > 0 &&
                      orders.every((order) => selected[order.id])
                    }
                    onChange={(event) => {
                      const checked = event.target.checked;
                      setSelected(
                        Object.fromEntries(
                          orders.map((order) => [order.id, checked]),
                        ),
                      );
                    }}
                  />
