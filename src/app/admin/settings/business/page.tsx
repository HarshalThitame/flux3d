  ...
  return value
    .map((tier) => {
      ...
      const minCartValue = Number(
        record.minCartValue ?? record.min_cart_value ?? 0,
      );
      const discountPercent = Number(
        record.discountPercent ?? record.discount_percent ?? 0,
      );
...
export default function BusinessSettingsPage() {
  ...
  const handleImport = useCallback(() => {
    ...
    input.onchange = async (e) => {
      ...
      try {
        ...
        setToast({
          type: "success",
          message: "Settings imported. Save to persist.",
        });
      } catch {
  ...
  const handleImageUpload = useCallback(
    async (field: string, file: File) => {
      ...
      try {
        ...
        setToast({
          type: "success",
          message: `${field.replace(/([A-Z])/g, " $1").trim()} updated.`,
        });
      } catch (err) {
        setToast({
          type: "error",
  ...
  const fb = (key: string): boolean =>
    bool(form[key] as boolean | null | undefined);
  const fn = (key: string): number =>
    num(form[key] as number | null | undefined);
  const cartDiscountTiers = normalizeCartDiscountTiers(form.cartDiscountTiers);

  const updateCartDiscountTier = useCallback(
    (index: number, field: keyof CartDiscountTier, value: number) => {
  ...
  const deleteCartDiscountTier = useCallback(
    (index: number) => {
      updateField(
        "cartDiscountTiers",
        cartDiscountTiers.filter((_, tierIndex) => tierIndex !== index),
      );
    },
    [cartDiscountTiers, updateField],
  );
  ...
  return (
    <>
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          <div>
            ...
            <p className="mt-1 max-w-xl text-sm text-[#6F7192]">
              Manage your company details, branding, invoices, and operational
              settings.
            </p>
          </div>
      ...
      <div
        className={`fixed bottom-0 left-0 right-0 z-50 border-t border-gray-200 bg-white/95 backdrop-blur-xl transition-all md:left-[280px] ${savingToast ? "" : ""}`}
      >
        <div className="mx-auto flex max-w-[1500px] items-center justify-between px-4 py-3 md:px-8">
          <div className="flex items-center gap-2">
            <div
              className={`h-2 w-2 rounded-full ${dirty ? "bg-amber-400" : "bg-emerald-400"}`}
            />
  ...
  return (
    ...
    >
      <div className="mb-5">
        ...
        {description && (
          <p className="mt-1 text-xs text-[#6F7192]">{description}</p>
        )}
      </div>
      {children}
    </motion.div>
  ...
  return (
    <div className="space-y-6">
      <SectionCard
        ...
        description="Legal and public-facing business details."
      >
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <InputField
            ...
            value={f("legalBusinessName")}
            onChange={(v) => updateField("legalBusinessName", v)}
            placeholder="Flux3D Pvt. Ltd."
          />
          <InputField
            label="Brand Name"
            value={f("brandName")}
          ...
          <InputField
            ...
            placeholder="3D Printing Made Easy"
          />
        </div>
        <div className="mt-4">
          <TextAreaField
        ...
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <InputField
            ...
            value={f("cinNumber")}
            onChange={(v) => updateField("cinNumber", v)}
            placeholder="U12345KA2020PTC123456"
          />
          <InputField
            label="MSME Registration Number (optional)"
            value={f("msmeNumber")}
            onChange={(v) => updateField("msmeNumber", v)}
            placeholder="UDYAM-KA-01-0000000"
          />
          <SelectField
            label="Business Type"
      ...
      <SectionCard
        ...
        description="Email addresses and phone numbers for various departments."
      >
        <div className="grid gap-4 md:grid-cols-2">
          <InputField
            label="Primary Email"
            type="email"
            value={f("primaryEmail")}
            onChange={(v) => updateField("primaryEmail", v)}
            placeholder="hello@flux3d.in"
          />
          <InputField
            label="Support Email"
            type="email"
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <div className="relative">
            ...
            {f("primaryPhone") && (
              <button
                type="button"
                onClick={() => copyToClipboard(f("primaryPhone"))}
                className="absolute right-3 top-8 text-[#6F7192] hover:text-[#0F1B3D]"
              >
          ...
          <div className="relative">
            ...
            {f("whatsappNumber") && (
              <button
                type="button"
                onClick={() => copyToClipboard(f("whatsappNumber"))}
                className="absolute right-3 top-8 text-[#6F7192] hover:text-[#0F1B3D]"
              >
                <Copy className="h-3.5 w-3.5" />
              </button>
          ...
          />
          <InputField
            label="Toll-Free Number (optional)"
            type="tel"
            value={f("tollFreeNumber")}
            onChange={(v) => updateField("tollFreeNumber", v)}
      ...
      <SectionCard
        ...
        description="Office and billing addresses."
      >
        <div className="space-y-4">
          ...
          <Divider />
          <ToggleField
            label="Billing address same as office address"
            description="Use the same address for billing purposes."
            checked={billingSame}
            onChange={(v) => updateField("billingSameAsOffice", v)}
          />
          {!billingSame && (
            <motion.div
            ...
            >
              <div className="text-xs font-semibold uppercase tracking-[0.15em] text-[#6F7192]">
                Billing Address
              </div>
              <InputField
                label="Billing Address Line 1"
                value={f("billingAddressLine1")}
                onChange={(v) => updateField("billingAddressLine1", v)}
              />
              <div className="grid gap-4 md:grid-cols-2">
                <InputField
                  label="Billing Address Line 2"
                  value={f("billingAddressLine2")}
                  onChange={(v) => updateField("billingAddressLine2", v)}
                />
              </div>
              <div className="grid gap-4 md:grid-cols-4">
                <InputField
                ...
                <InputField
                  label="State"
                  value={f("billingState")}
                  onChange={(v) => updateField("billingState", v)}
                />
                <InputField
                  label="Country"
                  value={f("billingCountry")}
  ...
  return (
    <div className="space-y-6">
      <SectionCard
        ...
        description="Upload logos and icons used across the website, invoices, and emails."
      >
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {imageFields.map(({ key, label }) => (
            ...
            >
              <div className="mb-2 text-xs font-medium text-[#aeb8d8]">
                {label}
              </div>
              {f(key) ? (
              ...
              <div className="mt-1.5 text-[9px] text-[#6F7192]">
                PNG, WebP, SVG. Max 2MB
              </div>
            </div>
          ))}
        </div>
      ...
      <SectionCard
        title="Brand Colors"
        description="Primary and secondary brand colors used across the UI and exports."
      >
        <div className="grid gap-4 md:grid-cols-2">
          <div>
            <div className="mb-1.5 text-xs font-medium text-[#aeb8d8]">
              Primary Color
            </div>
            <div className="flex gap-3">
              <input
                type="color"
                value={f("primaryColor") || "#6d28d9"}
...
function InvoicingTab({
  ...
  uploading,
  cartDiscountTiers,
  onAddCartDiscountTier,
  onDeleteCartDiscountTier,
  onUpdateCartDiscountTier,
}: {
  ...
  return (
    <div className="space-y-6">
      <SectionCard
        ...
        description="Configure how invoices and quotations are generated."
      >
        <div className="grid gap-4 md:grid-cols-3">
          <InputField
          ...
          />
          <InputField
            label="3D Shop Invoice Prefix"
            value={f("shopInvoicePrefix")}
            onChange={(v) => updateField("shopInvoicePrefix", v)}
            placeholder="SHP-"
          />
          <InputField
            label="Quotation Prefix"
            value={f("quotationPrefix")}
            onChange={(v) => updateField("quotationPrefix", v)}
            placeholder="QTN-"
          />
          <InputField
            label="Starting Invoice Number"
            type="number"
            value={String(fn("invoiceStartNumber"))}
            onChange={(v) => updateField("invoiceStartNumber", Number(v))}
          />
          <InputField
            label="Starting 3D Shop Invoice Number"
            type="number"
            value={String(fn("shopInvoiceStartNumber"))}
            onChange={(v) => updateField("shopInvoiceStartNumber", Number(v))}
          />
          <InputField
            label="Starting Quotation Number"
            type="number"
            value={String(fn("quotationStartNumber"))}
            onChange={(v) => updateField("quotationStartNumber", Number(v))}
          />
          <SelectField
          ...
          <SelectField
            ...
            value={f("currencySymbol")}
            onChange={(v) => updateField("currencySymbol", v)}
          />
          <InputField
            label="Tax Percentage (%)"
            type="number"
            value={String(fn("taxPercentage"))}
            onChange={(v) => updateField("taxPercentage", Number(v))}
          />
          <InputField
          ...
          />
          <ToggleField
            label="Enable Precise Slicer (A2L)"
            description="Use remote OrcaSlicer service for exact quote times and AMS color weight breakdown."
            checked={fb("slicerServiceEnabled")}
            onChange={(v) => updateField("slicerServiceEnabled", v)}
          />
          <InputField
            label="Slicer Service URL"
            value={f("slicerServiceUrl")}
            onChange={(v) => updateField("slicerServiceUrl", v)}
            placeholder="http://your-oracle-vm:8080"
          />
          <InputField
            label="AMS Color Change Surcharge (₹)"
            type="number"
            value={f("amsColorChangeSurcharge")}
            onChange={(v) =>
              updateField("amsColorChangeSurcharge", Number(v) || 30)
            }
            placeholder="30"
          />
          <InputField
            label="SAC / HSN Code"
            value={f("sacHsnCode")}
            onChange={(v) => updateField("sacHsnCode", v)}
            placeholder="9983"
          />
        </div>
        <Divider />
        <TextAreaField
          ...
          rows={2}
          placeholder="Payment due within 15 days from invoice date."
        />
      </SectionCard>

      <SectionCard
        title="Post-processing Multipliers"
        description="Configure finishing charges as multipliers applied to item production cost and difficulty."
      >
        <div className="grid gap-4 md:grid-cols-3">
          {[
            { key: "none", label: "None" },
            ...
            const multipliers = form.postProcessingMultipliers as
            ...
            return (
              <InputField
                ...
                onChange={(v) =>
                  updateField("postProcessingMultipliers", {
                    ...(multipliers ?? {}),
                    [option.key]: Number(v),
                  })
                }
      ...
      <SectionCard
        title="Cart Discount"
        description="Configure value-based discount tiers for instant quote pricing."
      >
        <div className="space-y-4">
          <ToggleField
            ...
            <div className="divide-y divide-gray-200 bg-white">
              {cartDiscountTiers.length === 0 ? (
                ...
                cartDiscountTiers.map((tier, index) => (
                  <div
                    key={`${tier.minCartValue}-${tier.discountPercent}-${index}`}
      ...
      <SectionCard
        title="Bank Details"
        description="Bank account information for payments and refunds."
      >
        <div className="grid gap-4 md:grid-cols-2">
          <InputField
            ...
            {f("upiQrCodeUrl") ? (
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <button
                    type="button"
                    onClick={() => triggerFileInput("upiQrCodeUrl")}
                    disabled={uploading === "upiQrCodeUrl"}
                  ...
                  <button
                    type="button"
                    onClick={() => updateField("upiQrCodeUrl", "")}
  ...
  return (
    <div className="space-y-6">
      <SectionCard
        ...
        description="Configure WhatsApp numbers and message templates."
      >
        <div className="grid gap-4 md:grid-cols-2">
          <div className="relative">
            <InputField
              label="WhatsApp Order Notification Number"
              type="tel"
              value={f("whatsappOrderNumber")}
              onChange={(v) => updateField("whatsappOrderNumber", v)}
              placeholder="+91 98765 43210"
            />
            {f("whatsappOrderNumber") && (
              <button
                type="button"
                onClick={() => copyToClipboard(f("whatsappOrderNumber"))}
                className="absolute right-3 top-8 text-[#6F7192] hover:text-[#0F1B3D]"
              >
                <Copy className="h-3.5 w-3.5" />
              </button>
          ...
          <div className="relative">
            <InputField
              ...
              type="tel"
              value={f("whatsappSupportNumber")}
              onChange={(v) => updateField("whatsappSupportNumber", v)}
              placeholder="+91 98765 43210"
            />
            {f("whatsappSupportNumber") && (
              <button
                type="button"
                onClick={() => copyToClipboard(f("whatsappSupportNumber"))}
                className="absolute right-3 top-8 text-[#6F7192] hover:text-[#0F1B3D]"
              >
                <Copy className="h-3.5 w-3.5" />
              </button>
        ...
        <div className="space-y-4">
          <div>
            <TextAreaField
              ...
              value={f("defaultWhatsappTemplate")}
              onChange={(v) => updateField("defaultWhatsappTemplate", v)}
              rows={3}
              placeholder="Hi {name}, your order #{orderNumber} has been received..."
            />
            <div className="mt-1 flex justify-between text-[10px] text-[#6F7192]">
              <span>
                Available variables: {"{name}"}, {"{orderNumber}"},{" "}
                {"{orderStatus}"}
              </span>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <div className="text-sm font-semibold text-[#0F1B3D]">
                WhatsApp RAG knowledge
              </div>
              <p className="mt-1 text-xs text-[#6F7192]">
                Manage the knowledge chunks that the WhatsApp AI assistant uses
                for grounded replies.
              </p>
            </div>
            <Link
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          />
          <TextAreaField
            label="Support Availability Message"
            value={f("supportAvailabilityMessage")}
            onChange={(v) => updateField("supportAvailabilityMessage", v)}
            rows={2}
            placeholder="We are available during business hours..."
          />
        </div>
      </SectionCard>
    </div>
...
function SEOTab({
  ...
  return (
    <div className="space-y-6">
      <SectionCard
        title="SEO & Metadata"
        description="Search engine optimization settings for your website."
      >
        <div className="space-y-4">
          <InputField
            label="Meta Title"
            ...
            placeholder="Flux3D - Professional 3D Printing Services"
          />
          <TextAreaField
            ...
            rows={2}
            placeholder="Professional 3D printing services..."
          />
          <InputField
            label="Meta Keywords"
            value={f("metaKeywords")}
            onChange={(v) => updateField("metaKeywords", v)}
            placeholder="3D printing, prototyping, manufacturing"
          />
          <InputField
            label="Canonical URL"
            type="url"
            ...
            placeholder="https://flux3d.in"
          />
          <ToggleField
            label="Robots Index"
            description="Allow search engines to index your site."
            checked={fb("robotsIndex")}
            onChange={(v) => updateField("robotsIndex", v)}
          />
        </div>
        <Divider />
        <div className="grid gap-4 md:grid-cols-2">
            ...
            {f("ogImageUrl") ? (
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <button
                    type="button"
                    onClick={() => triggerFileInput("ogImageUrl")}
                    className="rounded-lg border border-gray-200 bg-gray-50 px-3 py-1.5 text-[10px] text-[#6F7192] hover:bg-gray-200"
                  >
                    Replace
                  </button>
                  <button
          ...
          <div>
            <div className="mb-1.5 text-xs font-medium text-[#aeb8d8]">
              Twitter Meta Image (1200x600)
            </div>
            {f("twitterImageUrl") ? (
              <div className="space-y-2">
                <div className="flex items-center justify-center rounded-lg border border-gray-200 bg-gray-50/50 p-2">
                  <Image
                    src={f("twitterImageUrl")}
                ...
                <div className="flex gap-2">
                  <button
                    type="button"
                    onClick={() => triggerFileInput("twitterImageUrl")}
                    className="rounded-lg border border-gray-200 bg-gray-50 px-3 py-1.5 text-[10px] text-[#6F7192] hover:bg-gray-200"
                  >
                    Replace
                  </button>
                  <button
                    type="button"
                    onClick={() => updateField("twitterImageUrl", "")}
                    className="rounded-lg border border-rose-200 bg-rose-50 px-3 py-1.5 text-[10px] text-rose-600 hover:bg-rose-400/15"
                  >
                    Remove
                  </button>
              ...
              <button
                type="button"
                onClick={() => triggerFileInput("twitterImageUrl")}
                disabled={uploading === "twitterImageUrl"}
                className="flex w-full items-center justify-center rounded-lg border border-dashed border-gray-300 bg-gray-50/50 py-8 text-[11px] text-[#6F7192] hover:border-gray-400 hover:bg-gray-50 disabled:opacity-50"
              >
                {uploading === "twitterImageUrl"
                  ? "Uploading..."
                  : "Click to upload"}
              </button>
            )}
  ...
  return (
    <div className="space-y-6">
      <SectionCard
        ...
        description="SMTP settings for sending transactional emails. This section is optional."
      >
        <div className="grid gap-4 md:grid-cols-2">
          <InputField
            label="SMTP Host"
          ...
          <InputField
            label="SMTP Port"
            type="number"
            value={String(fn("smtpPort") || "")}
            onChange={(v) => updateField("smtpPort", Number(v))}
            placeholder="587"
          />
          <InputField
            label="SMTP Username"
            value={f("smtpUsername")}
            onChange={(v) => updateField("smtpUsername", v)}
          ...
          <InputField
            label="Sender Name"
            value={f("smtpSenderName")}
            onChange={(v) => updateField("smtpSenderName", v)}
            placeholder="Flux3D"
          />
          <InputField
            label="Sender Email"
            type="email"
            value={f("smtpSenderEmail")}
            onChange={(v) => updateField("smtpSenderEmail", v)}
            placeholder="noreply@flux3d.in"
          />
        </div>
        <Divider />
        <button
  ...
  return (
    <div className="space-y-6">
      <SectionCard
        ...
        description="Links to your legal pages."
      >
        <div className="grid gap-4 md:grid-cols-2">
          <InputField
            ...
            placeholder="https://flux3d.in/privacy"
          />
          <InputField
            ...
            placeholder="https://flux3d.in/terms"
          />
          <InputField
            label="Refund Policy URL"
            type="url"
            value={f("refundPolicyUrl")}
            onChange={(v) => updateField("refundPolicyUrl", v)}
            placeholder="https://flux3d.in/refund"
          />
          <InputField
            label="Shipping Policy URL"
            type="url"
            value={f("shippingPolicyUrl")}
            onChange={(v) => updateField("shippingPolicyUrl", v)}
            placeholder="https://flux3d.in/shipping"
          />
        </div>
      </SectionCard>

      <SectionCard
        title="Business Operational Settings"
        description="Configure business operations, working hours, and delivery."
      >
        <div className="grid gap-4 md:grid-cols-2">
          <SelectField
            label="Working Days"
          ...
          <TextAreaField
            ...
            value={f("holidayMessage")}
            onChange={(v) => updateField("holidayMessage", v)}
            rows={2}
            placeholder="We are closed on public holidays."
          />
          <InputField
          ...
          <InputField
            ...
            type="number"
            value={String(fn("deliveryChargeThreshold"))}
            onChange={(v) => updateField("deliveryChargeThreshold", Number(v))}
          />
          <InputField
            label="Shop Minimum Order Value (₹)"
            type="number"
            value={String(fn("shopMinimumOrderValue"))}
            onChange={(v) => updateField("shopMinimumOrderValue", Number(v))}
          />
          <InputField
            label="Default Delivery Charge (₹)"
            type="number"
            value={String(fn("defaultDeliveryCharge"))}
            onChange={(v) => updateField("defaultDeliveryCharge", Number(v))}
          />
          <div className="md:col-span-2">
            <div className="grid gap-4 md:grid-cols-2">
              <ToggleField
                ...
                checked={fb("pickupAvailable")}
                onChange={(v) => updateField("pickupAvailable", v)}
              />
            </div>
          </div>
        </div>
