...
export default function OrderDetailClient({ initialOrder }: Props) {
  ...
  const postProcessingCharges = order.items.reduce(
    (sum, item) => sum + item.postProcessingCharges,
    0,
  );
  ...
  const totalPrintTime = order.items.reduce(
    (sum, item) => sum + item.estimatedTime,
    0,
  );
  const discountTotal =
    order.discountAmount ??
    order.cartDiscountAmount +
      order.couponDiscountAmount +
      order.offerDiscountAmount;
  const addressLines = [
    order.addressLine1,
    order.addressLine2,
  ...
  async function updateStatus(status: OrderStatus, reason?: string) {
    ...
    if (!isSequentialOrderStatusTransition(order.status, status)) {
      showToast({
        type: "error",
        message: getOrderStatusTransitionError(order.status, status),
      });
      return;
    }
    ...
    try {
      const response = await fetch("/api/admin/orders", {
        ...
        body: JSON.stringify({
          ...
          ...(status === "shipped" && trackingForm.courier_name
            ? { courier_name: trackingForm.courier_name }
            : {}),
          ...(status === "shipped" && trackingForm.tracking_number
            ? { tracking_number: trackingForm.tracking_number }
            : {}),
          ...(status === "shipped" && trackingForm.tracking_url
      ...
      setOrder((current) => ({
        ...
        providerPaymentId:
          json.order.providerPaymentId ?? current.providerPaymentId,
        paymentMethod: json.order.paymentMethod ?? current.paymentMethod,
        paymentVerifiedAt:
          json.order.paymentVerifiedAt ?? current.paymentVerifiedAt,
        paymentFailedAt: json.order.paymentFailedAt ?? current.paymentFailedAt,
        paymentRefundStatus:
          json.order.paymentRefundStatus ?? current.paymentRefundStatus,
        paymentRefundAmountPaise:
          json.order.paymentRefundAmountPaise ??
          current.paymentRefundAmountPaise,
        paymentAttemptId:
          json.order.paymentAttemptId ?? current.paymentAttemptId,
      }));
      ...
      showToast({
        type: "success",
        message: `${json.order.orderNumber} marked ${STATUS_LABELS[status].toLowerCase()}.`,
      });
    } catch (error) {
      setOrder(previous);
      showToast({
          ...
          <div className="w-full overflow-hidden rounded-xl border border-gray-200 bg-white p-4 shadow-sm sm:p-6">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div className="min-w-0 space-y-3">
                <Link
                  href="/admin/orders"
                  className="inline-flex items-center gap-2 text-sm font-medium text-gray-600 transition hover:text-violet-700"
                >
                ...
                <div className="flex min-w-0 flex-wrap items-center gap-3">
                  ...
                  <span
                    className={`inline-flex rounded-full border px-2.5 py-1 text-xs font-semibold uppercase tracking-wide ${statusPillClass(order.status)}`}
                  >
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <button
                  ...
                  disabled={
                    updatingStatus || !canCancelOrderStatus(order.status)
                  }
          ...
          <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-5">
            <div className="min-w-0 space-y-6 lg:col-span-3">
              ...
              <Card title="Pricing Breakdown">
                <PricingTable
                  order={order}
                  postProcessingCharges={postProcessingCharges}
                ...
                <div className="space-y-4">
                  {getTimelineSteps(order).map((step, index) => (
                    <TimelineStep
                      key={step.label}
                      label={step.label}
                      meta={timelineMeta(order, step, index)}
                      state={timelineState(order.status, step, index)}
                    />
                  ))}
                </div>
              </Card>
              ...
              <Card title="Order Meta">
                <div className="space-y-3">
                  ...
                  <MetaRow
                    ...
                    action={
                      <button
                        type="button"
                        onClick={() =>
                          copyToClipboard("Group ID", order.groupId)
                        }
                  ...
                  <MetaRow
                    ...
                    value={safeText(order.paymentStatus ?? "Pending")}
                  />
                  <MetaRow
                    label="Items"
                    value={`${itemCount} item${itemCount === 1 ? "" : "s"} · ${totalQuantity.toLocaleString("en-IN")} pcs`}
                  />
                  <MetaRow
                    label="Print Time"
                    value={`${formatNumber(totalPrintTime, 1)} hrs`}
                  />
                  <MetaRow
                    label="Created"
                ...
                <Card title="Payment Details">
                  <div className="space-y-3">
                    ...
                    {order.paymentStatus && (
                      <MetaRow
                        label="Status"
                        value={safeText(order.paymentStatus)}
                      />
                    )}
                    ...
                    {order.paymentVerifiedAt && (
                      <MetaRow
                        label="Verified At"
                        value={formatDateTime(order.paymentVerifiedAt)}
                      />
                    )}
                    {order.paymentFailedAt && (
                      <MetaRow
                        label="Failed At"
                        value={formatDateTime(order.paymentFailedAt)}
                      />
                    )}
                    {order.paymentRefundStatus && (
                      <>
                        <MetaRow
                          label="Refund Status"
                          value={safeText(order.paymentRefundStatus)}
                        />
                        {order.paymentRefundAmountPaise != null &&
                          order.paymentRefundAmountPaise > 0 && (
                            <MetaRow
                              label="Refund Amount"
                              value={`₹${(order.paymentRefundAmountPaise / 100).toLocaleString("en-IN", { minimumFractionDigits: 2 })}`}
                            />
                          )}
                      </>
                    )}
                      ...
                      {order.paymentRefundStatus ? (
                        ...
                        <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-700">
                          <div className="font-semibold">
                            Prepaid order cancelled
                          </div>
                          <p className="mt-1 leading-6">
                            This order was paid but has no refund initiated yet.
                            Process the refund from the payment details page.
                          </p>
                          {order.paymentAttemptId && (
                            <a
                              href={`/admin/payments/${order.paymentAttemptId}`}
                ...
                {order.status === "shipped" &&
                  (!order.tracking_number || !order.courier_name) && (
                    <div className="mt-3 rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-700">
                      ...
                      Order is marked as shipped but tracking info is missing.
                      The customer will not receive a tracking email.
                    </div>
                  )}
              </Card>
  ...
  return (
    <article className="w-full overflow-hidden rounded-xl border border-gray-200 bg-white p-4 shadow-sm sm:p-6">
      ...
      <div className="mt-4 grid grid-cols-2 gap-3 md:grid-cols-4">
        ...
        <Spec
          label="Layer Height"
          value={`${formatNumber(item.layerHeight)} mm`}
        />
        <Spec
          label="Quantity"
          value={`${formatNumber(item.quantity, 0)} pcs`}
        />
        <Spec label="Supports" value={item.supports ? "Required" : "None"} />
        <Spec
          label="Post-process"
          value={postProcessingLabel(item.postProcessingLevel)}
        />
        <Spec
          label="Difficulty"
          value={`${formatNumber(item.difficultyFactor, 2)}x`}
        />
        <Spec label="Dimensions" value="—" className="md:col-span-2" />
        <Spec
          label="Weight"
          value={`${formatNumber(item.weight, 2)} g/unit`}
      ...
      <div className="mt-4 border-t border-gray-100 pt-4">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div className="min-w-0">
            ...
            <div
              className="mt-1 max-w-full truncate text-xs text-gray-500"
              title={item.fileUrl ?? undefined}
            >
    ...
    <div className="w-full overflow-hidden">
      <table className="w-full table-fixed text-sm">
        <tbody>
          ...
          <PricingRow
            label="Machine Cost"
            value={formatMoney(order.machineCost)}
          />
          <PricingRow
            label="Post-processing"
            value={formatMoney(postProcessingCharges)}
          />
          <PricingDivider />
          <PricingRow label="Subtotal" value={formatMoney(order.subtotal)} />
          <PricingRow
            label={`Overhead (${formatNumber(order.overheadPercentage, 2)}%)`}
            value={formatMoney(order.overheadAmount)}
          />
          <PricingRow
            label={`Margin (${formatNumber(order.marginPercentage, 2)}%)`}
            value={formatMoney(order.marginAmount)}
          />
          <PricingDivider />
          <PricingRow
            label="Total Price"
            value={formatMoney(order.totalPriceBeforeDiscount)}
            strong
          />
          ...
          <PricingRow
            label="Final Price"
            value={formatMoney(order.finalPrice)}
            tone="success"
            strong
          />
          <PricingRow
...
function PricingRow({
  ...
  return (
    <tr>
      <td
        className={`w-3/5 truncate py-2 pr-4 text-gray-600 ${tone === "grand" ? "text-base font-semibold text-gray-900" : ""}`}
        title={label}
      >
        {label}
      </td>
  ...
  return (
    <tr>
      <td colSpan={2} className="py-2">
        <div
          className={
            heavy ? "border-t-2 border-gray-200" : "border-t border-gray-100"
          }
        />
      </td>
    </tr>
...
function extractQuoteId(order: AdminOrder) {
  const fileUrl =
    order.fileUrl ?? order.items.find((item) => item.fileUrl)?.fileUrl ?? "";
...
function timelineMeta(
  ...
  const timestamp =
    order.statusTimestamps?.[step.status] ??
    (step.status === "pending" ? order.createdAt : null);
...
function Spec({
  ...
  return (
    <div
      className={`min-w-0 rounded-lg border border-gray-100 bg-gray-50 px-3 py-2 ${className}`}
    >
      <div className="truncate text-xs text-gray-400">{label}</div>
      <div className="mt-1 min-w-0 text-sm font-medium text-gray-900">
        {value}
      </div>
    </div>
  );
...
function InfoLine({
  ...
  return (
    <div className="flex min-w-0 items-center gap-3 text-sm text-gray-600">
      ...
      {children ? (
        <span
          className={`min-w-0 truncate ${strong ? "font-semibold text-gray-900" : undefined}`}
        >
          {children}
        </span>
      ) : (
...
function DiscountRow({
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-gray-50 p-4">
      <div className="flex items-start gap-3">
        <span className="rounded-lg border border-orange-200 bg-orange-50 p-2 text-orange-500">
          {icon}
        </span>
  ...
  return (
    <div className="flex min-w-0 items-center justify-between gap-4 border-b border-gray-100 pb-3 last:border-b-0 last:pb-0">
      ...
      <span className="flex min-w-0 items-center gap-2 text-right text-sm font-medium text-gray-900">
        <span className="truncate" title={safeText(value)}>
          {safeText(value)}
        </span>
        {action}
      </span>
