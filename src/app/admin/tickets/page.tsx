...
export default function TicketsPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const json = (await response.json()) as { tickets: SupportTicket[]; total: number; page: number; limit: number }
  ...
  }, [activeStatus, activeSource, searchQuery, page, refreshKey])

  const { status: realtimeStatus } = useTicketsRealtime(() => {
    setRefreshKey((k) => k + 1)
  })
    ...
    <div className="space-y-6">
      ...
      >
        <TicketSearch
          ...
          placeholder="Search by ticket number, customer, subject, message content..."
        />
      </motion.div>
      ...
      >
        {sourceTabs.map((tab) => (
          <button
            key={tab.value}
            onClick={() => { setActiveSource(tab.value); setPage(1) }}
