  ...
  const builder: Record<string, unknown> = {
    ...
    insert: vi.fn(() => {
      const insertBuilder: Record<string, unknown> = {
        select: vi.fn(() => ({
          maybeSingle: vi.fn().mockResolvedValue({ data: { id: 'test-event-id' }, error: null }),
        })),
      }
...
function createMockReqRes(rawBody: string) {
  const signature = 'sha256=' + crypto.createHmac('sha256', TEST_SECRET).update(rawBody, 'utf8').digest('hex')
...
describe('WhatsApp conversation memory', () => {
  ...
  describe('saveSession', () => {
    ...
    it('handles rpc error gracefully', async () => {
      ...
      await expect(saveSession(mockSupabase, '+919999999991', 'Hello', 'Hi')).resolves.not.toThrow()
    })
  })
  ...
  describe('history injection into generateWhatsAppReply', () => {
    it('injects history into the OpenAI messages array', async () => {
      ...
      await generateWhatsAppReply('What is PETG?', FALLBACK_SETTINGS as never, 'PETG knowledge', history)
    ...
    it('adjusts max_tokens based on history length', async () => {
      ...
      const longHistory: Array<ChatCompletionMessageParam> = Array.from({ length: 8 }, (_, i) => ({
        role: (i % 2 === 0 ? 'user' : 'assistant') as 'user' | 'assistant',
        content: 'A'.repeat(500),
      }))

      await generateWhatsAppReply('Short message', FALLBACK_SETTINGS as never, '', longHistory)
  ...
  describe('session loading from handler (integration)', () => {
    ...
    it('filters out malformed history entries', async () => {
      ...
      const historyEntries = callArg.messages.filter(
        (m: ChatCompletionMessageParam) => m.role !== 'system' && m.content !== 'Customer message:\nTell me more'
      )
    ...
    it('slices history to WHATSAPP_SESSION_TURNS * 2 entries', async () => {
      ...
      expect(historyEntries[3]).toEqual({ role: 'assistant', content: 'Message 10' })
    })
    ...
    it('handles missing session gracefully', async () => {
      ...
      const callArg = chatCompletionsCreate.mock.calls[0][0]
      const userMessages = callArg.messages.filter((m: ChatCompletionMessageParam) => m.role === 'user')
