...
vi.mock("openai", () => ({
  default: class MockOpenAI {
    ...
    embeddings = {
      create: vi.fn().mockResolvedValue({ data: [{ embedding: [1, 0, 0] }] }),
    };
  },
}));
...
function makeBuilder() {
  const builder: Record<string, unknown> = {
    ...
    maybeSingle: vi.fn(() => {
      ...
      if (builder._table === "whatsapp_sessions")
...
vi.mock("@/lib/whatsapp-rag", () => ({
  getWhatsAppRagContext: vi.fn(() =>
    Promise.resolve({
      ...
      sources: [
        { sourceKey: "test", title: "Test", score: 0.95, content: "knowledge" },
      ],
      mode: "database",
      confidence: 0.95,
    }),
...
vi.mock("@/lib/whatsapp-price-validation", () => ({
  validatePricesInResponse: vi.fn(() => ({
    valid: true,
    safeResponse: "",
    mentionedPrices: [],
    hallucinatedPrices: [],
...
describe("WhatsApp webhook handler", () => {
  beforeEach(() => {
    ...
    const { resetPendingAsyncWork } = vi.hoisted(() => ({
  ...
  it("handles out_of_scope intent with fast path", async () => {
    ...
    vi.mocked(classifierMod.classifyIntent).mockResolvedValue({
      intent: "out_of_scope",
      keywords: [],
    });
  ...
  it("processes greeting intent", async () => {
    ...
    chatCompletionsCreate.mockResolvedValue({
      choices: [
        { message: { content: "Hello! Welcome to Flux3D. How can I help?" } },
      ],
      model: "gpt-4.1-mini",
