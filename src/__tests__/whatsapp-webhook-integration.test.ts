...
const testState = vi.hoisted(() => {
  ...
  let maybeSingleResult: { data: Record<string, unknown> | null; error: null } = { data: null, error: null }
  let selectResult: { data: Array<Record<string, unknown>> | null; error: null } = { data: [], error: null }
  let insertResult: { data: Array<Record<string, unknown>> | null; error: null } = { data: [{ id: 'evt-1' }], error: null }
  let updateResult = { error: null }
  let rpcResult: { data: Array<Record<string, unknown>> | null; error: null } = { data: [], error: null }

  return {
    chatCompletionMock, embeddingMock, fetchMock, auditLogMock,
    rateLimitMock, settingsMock, ragContextMock, classifyIntentMock,
    validatePricesMock, structuredDataMock,
    get supabaseRows() { return supabaseRows },
    set supabaseRows(v) { supabaseRows = v },
    get maybeSingleResult() { return maybeSingleResult },
    set maybeSingleResult(v) { maybeSingleResult = v },
    get selectResult() { return selectResult },
    set selectResult(v) { selectResult = v },
    get insertResult() { return insertResult },
    set insertResult(v) { insertResult = v },
    get updateResult() { return updateResult },
    set updateResult(v) { updateResult = v },
    get rpcResult() { return rpcResult },
    set rpcResult(v) { rpcResult = v },
  }
})
...
function makeBuilder(table: string) {
  ...
  const builder: Record<string, unknown> = {
    ...
    insert: vi.fn(() => ({ select: vi.fn(() => ({ maybeSingle: vi.fn(() => Promise.resolve(testState.insertResult)) })) })),
    update: vi.fn(() => ({ eq: vi.fn(() => Promise.resolve(testState.updateResult)) })),
...
vi.mock('@supabase/supabase-js', () => ({
  createClient: vi.fn(() => ({
    ...
    auth: {
      ...
      getSession: vi.fn().mockResolvedValue({ data: { session: { user: { id: 'admin-1', email: 'admin@flux3d.in' } } }, error: null }),
      signOut: vi.fn(),
    },
  })),
}))
...
const RATE_LIMIT_PASS = { success: true, limit: 20, remaining: 19, reset: Date.now() + 60000 }

const DEFAULT_RAG = { context: '', sources: [], mode: 'none' as const, confidence: 0 }

describe('WhatsApp Webhook Integration', () => {
  let processIncomingMessage: any

  function expectWhatsAppSent() {
    const calls = testState.fetchMock.mock.calls.filter((c: any) =>
  ...
  beforeEach(async () => {
    ...
    testState.classifyIntentMock.mockReset().mockResolvedValue({ intent: 'general', keywords: [] })
    testState.validatePricesMock.mockReset().mockResolvedValue({ valid: true, safeResponse: '', mentionedPrices: [], hallucinatedPrices: [] })
    testState.structuredDataMock.mockReset().mockResolvedValue({
      materials: '', products: '', orderStatus: '',
      orderResults: [], totalMatches: 0,
      materialPrices: [], productPrices: [],
    })

    testState.chatCompletionMock.mockReset().mockResolvedValue({
      choices: [{ message: { content: 'Hi, thanks for reaching out!' }, finish_reason: 'stop' }],
      model: 'gpt-4.1-mini',
      usage: { prompt_tokens: 100, completion_tokens: 20, total_tokens: 120 },
    })
    testState.embeddingMock.mockReset().mockResolvedValue({ data: [{ embedding: [0.1, 0.2, 0.3] }] })

    testState.fetchMock.mockReset().mockResolvedValue({ ok: true, text: () => Promise.resolve('') })
    vi.spyOn(global, 'fetch').mockImplementation(testState.fetchMock)

    testState.maybeSingleResult = { data: { id: 'profile-1', phone_number: FROM_PHONE }, error: null }
    ...
    Object.keys(tableData).forEach((k) => { tableData[k].value = null })
  ...
  it('A — Price inquiry with RAG: generates reply, sends WhatsApp, logs audit', async () => {
    testState.ragContextMock.mockResolvedValue({
      ...
      sources: [{ sourceKey: 'materials_pla', title: 'PLA Filament', score: 0.92, content: 'PLA: ₹499/kg' }],
      mode: 'database',
      confidence: 0.92,
    })
    testState.chatCompletionMock.mockResolvedValue({
      choices: [{ message: { content: 'PLA filament is ₹499 per kilogram.', finish_reason: 'stop' } }],
      model: 'gpt-4.1-mini',
  ...
  it('B — Price hallucination: returns fallback with safe template', async () => {
    testState.ragContextMock.mockResolvedValue({
      ...
      sources: [{ sourceKey: 'materials_pla', title: 'PLA', score: 0.9, content: 'PLA: ₹499/kg' }],
      mode: 'database',
      confidence: 0.9,
    })
    testState.chatCompletionMock.mockResolvedValue({
      choices: [{ message: { content: 'PLA filament is ₹999 per kilogram.', finish_reason: 'stop' } }],
      model: 'gpt-4.1-mini',
      usage: { prompt_tokens: 120, completion_tokens: 15, total_tokens: 135 },
    })
    testState.structuredDataMock.mockResolvedValue({
      materials: '', products: '', orderStatus: '',
      orderResults: [], totalMatches: 1,
  ...
  it('C — Order status: returns order info via structured data', async () => {
    ...
    console.error('[test-debug-C] ragContextMock:', JSON.stringify(testState.ragContextMock.getMockImplementation?.() ?? 'no impl'))
    testState.classifyIntentMock.mockResolvedValue({ intent: 'order_status', keywords: ['order', 'status'] })
    testState.structuredDataMock.mockResolvedValue({
      materials: '', products: '', orderStatus: 'Your order #F3D-123 is being printed.',
      orderResults: [{ orderNumber: 'F3D-123', status: 'printing', createdAt: '2026-07-20' }],
      totalMatches: 1,
      materialPrices: [], productPrices: [],
    })
    testState.ragContextMock.mockResolvedValue({
      context: 'Order F3D-123: printing, estimated delivery Jul 25.',
      sources: [{ sourceKey: 'order_f3d123', title: 'Order F3D-123', score: 0.88, content: 'Printing, delivery Jul 25' }],
      mode: 'database',
      confidence: 0.88,
    })
    testState.chatCompletionMock.mockResolvedValue({
      choices: [{ message: { content: 'Your order F3D-123 is currently being printed and is estimated to ship by Jul 25.' } }],
      model: 'gpt-4.1-mini',
  ...
  it('D — Greeting: returns fallback reply without RAG', async () => {
    testState.classifyIntentMock.mockResolvedValue({ intent: 'greeting', keywords: ['hi'] })
  ...
  it('F — Out of scope: polite decline, no RAG query', async () => {
    testState.classifyIntentMock.mockResolvedValue({ intent: 'out_of_scope', keywords: [] })
    testState.ragContextMock.mockResolvedValue(DEFAULT_RAG)
  ...
  it('G — Session memory: includes prior conversation turns', async () => {
    const priorTurns = [
      ...
      { role: 'assistant', content: 'PLA is ₹499/kg. Would you like to upload a file?' },
    ]
    ...
    testState.chatCompletionMock.mockImplementation(async (opts: any) => {
      ...
      return {
        choices: [{ message: { content: 'Yes, please upload your file to get started.' } }],
        model: 'gpt-4.1-mini',
