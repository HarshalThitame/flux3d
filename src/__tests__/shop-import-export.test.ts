...
describe('csvRowToRecord / mapCsvRecord', () => {
  const headers = ['name', 'slug', 'base_price', 'tags', 'image_urls', 'is_active']

  it('maps a row to a record keyed by headers', () => {
    const record = csvRowToRecord(['Lamp', 'lamp', '1299', 'led|desk', 'a.jpg|b.jpg', 'true'], headers)
...
describe('productsToCsv', () => {
  it('produces a round-trippable CSV', () => {
    const csv = productsToCsv([
      {
        ...
        published_at: null,
        variants: [{ option_name: 'Color', option_type: 'swatch_color', values: ['Red'], is_required: true }],
        skus: [],
      },
    ])
    ...
    expect(rows[1][rows[0].indexOf('model_url')]).toBe('https://cdn.example.com/lamp.glb')
  })
})
