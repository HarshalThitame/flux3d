...
describe('revision storage', () => {
  ...
  it('caps the number of stored revisions', () => {
    for (let i = 0; i < 25; i += 1) {
      addRevision('product-1', makeRevision({ timestamp: i, product: { ...emptyProduct, name: `Name ${i}` } }))
    }
...
describe('isSameState', () => {
  ...
  it('treats nested key order as equal', () => {
    ...
    const a = makeRevision({ product: { ...emptyProduct, name: 'Lamp', image_alt: imageAltA } })
    const b = makeRevision({ product: { ...emptyProduct, name: 'Lamp', image_alt: imageAltB } })
...
describe('deepStableStringify', () => {
  it('is order-independent for nested objects and arrays', () => {
    ...
    expect(deepStableStringify(['a', 'b'])).toBe(deepStableStringify(['a', 'b']))
  })

  it('differs for different values', () => {
    expect(deepStableStringify({ a: 1 })).not.toBe(deepStableStringify({ a: 2 }))
  })
})
...
describe('describeChanges', () => {
  it('lists changed product fields', () => {
    ...
    const newer = makeRevision({ product: { ...older.product, base_price: 999 } })
    expect(describeChanges(older, newer)).toContain('Price')
  })

  it('reports added SKUs', () => {
    const older = makeRevision()
    const newer = makeRevision({ skus: [{ id: 's1' } as ShopRevision['skus'][number]] })
  ...
  it('reports edited SKU values when counts are unchanged', () => {
    const older = makeRevision({ skus: [{ id: 's1', price: 100 } as ShopRevision['skus'][number]] })
    const newer = makeRevision({ skus: [{ id: 's1', price: 150 } as ShopRevision['skus'][number]] })
