...
function DepthCard(props: CardProps) {
  const { src, alt, index, total, smoothScroll, dims, maxRotation, borderRadius, onSelect } = props

  const localOffset = useTransform(smoothScroll, (v) => wrapOffset(index - v, total))
  const absOffset = useTransform(localOffset, Math.abs)

  const cardWidth = useTransform(absOffset, [0, 1], [dims.itemWidth, dims.sideItemWidth], {
    clamp: true,
  })
  const cardHeight = useTransform(absOffset, [0, 1], [dims.itemHeight, dims.sideItemHeight], {
    clamp: true,
  })
    ...
    >
      <motion.div
        style={{
          ...
          overflow: 'hidden',
          background: '#efe9dc',
          boxShadow: '0 30px 60px -18px rgba(23,19,16,0.35), 0 12px 24px -12px rgba(23,19,16,0.25)',
          opacity,
          cursor: 'pointer',
...
export default function DepthBlurCarousel({
  ...
  useEffect(() => {
    ...
    const handler = (event: WheelEvent) => {
      ...
      const delta =
        Math.abs(event.deltaX) > Math.abs(event.deltaY) ? event.deltaX : event.deltaY * 0.8
  ...
  const handleKeyDown = useCallback(
    (event: React.KeyboardEvent<HTMLDivElement>) => {
      ...
      if (event.key === 'ArrowLeft' || event.key === 'ArrowRight') {
        event.preventDefault()
        scrollTarget.current = Math.round(scrollTarget.current) + (event.key === 'ArrowLeft' ? -1 : 1)
        rawScroll.set(scrollTarget.current)
      }
    },
