...
function productHasVariant(
  ...
  return product.variant_options.some(
    (option) =>
      option.option_name === optionName &&
      (option.values ?? []).some((value) => values.includes(value)),
  );
}
...
function FilterPanel({
  ...
  return (
    <div className="space-y-6">
      <div>
        <div className="mb-3 text-sm font-semibold text-[var(--shop-text-primary)]">
          Price
        </div>
        <Slider.Root
          value={selectedPrice}
          ...
          onValueCommit={([min, max]) =>
            onPriceChangeAction([min ?? priceRange[0], max ?? priceRange[1]])
          }
        ...
        <div key={name}>
          ...
          <div className="space-y-2">
            {values.map((value) => (
              <label
                key={value}
                className="flex min-h-[44px] items-center gap-3 rounded-xl border border-[var(--shop-border-light)] bg-white px-3 text-sm text-[var(--shop-text-secondary)] transition hover:border-[var(--shop-border-gold)]"
              >
                <input
                  type="checkbox"
      ...
      <label className="flex min-h-[44px] items-center gap-3 rounded-xl border border-[var(--shop-border-light)] bg-white px-3 text-sm font-semibold text-[var(--shop-text-secondary)] transition hover:border-[var(--shop-border-gold)]">
        <input
          type="checkbox"
          checked={inStockOnly}
          onChange={(event) => onInStockChangeAction(event.target.checked)}
          className="h-4 w-4 accent-[var(--shop-gold)]"
        />
        In Stock Only
      </label>
      ...
      <label className="flex min-h-[44px] items-center gap-3 rounded-xl border border-[var(--shop-border-light)] bg-white px-3 text-sm font-semibold text-[var(--shop-text-secondary)] transition hover:border-[var(--shop-border-gold)]">
        <input
          type="checkbox"
          checked={ratingOnly}
          onChange={(event) => onRatingChangeAction(event.target.checked)}
          className="h-4 w-4 accent-[var(--shop-gold)]"
        />
        4★ & above
      </label>
      ...
      <button
        type="button"
        onClick={clearAction}
        className="min-h-[44px] w-full rounded-xl border border-[var(--shop-border-light)] bg-white text-sm font-semibold text-[var(--shop-text-primary)] transition hover:border-[var(--shop-gold)] hover:text-[var(--shop-gold)]"
      >
        Clear All Filters
      </button>
    </div>
...
export default function ShopCategoryBrowser({
    ...
    products.forEach((product) => {
      product.variant_options.forEach((option) => {
        if (
          ...
          return;
        filters[option.option_name] = unique([
          ...(filters[option.option_name] ?? []),
          ...(option.values ?? []),
        ]);
      });
    });
  ...
  const filteredProducts = useMemo(() => {
    const next = products.filter((product) => {
      if (
        product.display_price < selectedPrice[0] ||
        product.display_price > selectedPrice[1]
      )
        return false;
      if (inStockOnly && !product.in_stock) return false;
      if (ratingOnly && product.avg_rating < 4) return false;
      return Object.entries(selectedVariants).every(([name, values]) =>
        productHasVariant(product, name, values),
      );
    });
    ...
    if (sort === "price_asc")
      next.sort((a, b) => a.display_price - b.display_price);
    if (sort === "price_desc")
      next.sort((a, b) => b.display_price - a.display_price);
  ...
  const chips = [
    ...
    ...Object.entries(selectedVariants).flatMap(([name, values]) =>
      ...
      <section className="relative min-h-[320px] overflow-hidden px-4 pb-12 pt-24 md:px-8 lg:px-16 lg:pt-32">
        ...
        <div className="relative z-10 mx-auto max-w-7xl">
          <nav
            ...
            className="mb-5 flex flex-wrap items-center gap-2 text-sm text-white"
          >
            <Link href="/" className="transition hover:opacity-80">
              Home
            </Link>
            <span aria-hidden="true">/</span>
            <Link href="/3d-shop" className="transition hover:opacity-80">
              3D Shop
            </Link>
            <span aria-hidden="true">/</span>
            ...
            {chips.length > 0 && (
              <div className="mb-5 flex flex-wrap gap-2">
                {chips.map((chip) => (
                  <span
                    key={chip}
                    className="inline-flex items-center gap-1 rounded-full border border-[var(--shop-border-gold)] bg-[var(--shop-gold-faint)] px-3 py-1 text-xs font-semibold text-[var(--shop-gold)]"
                  >
                    {chip}
                  </span>
                ))}
                <button
                  ...
                  className="text-xs font-semibold text-[var(--shop-text-muted)] transition hover:text-[var(--shop-gold)]"
                >
                  Clear
                </button>
              </div>
        ...
        <div className="fixed inset-0 z-[120] overflow-y-auto bg-[var(--shop-bg-base)] p-5 pb-[calc(1.25rem+env(safe-area-inset-bottom))] pt-[calc(1.25rem+env(safe-area-inset-top))] lg:hidden">
          <div className="mb-5 flex items-center justify-between">
            ...
            <button
              ...
              className="grid h-11 w-11 place-items-center rounded-xl border border-[var(--shop-border-light)] bg-white"
            >
              <X className="h-5 w-5 text-[var(--shop-text-secondary)]" />
            </button>
          </div>
          <div className="h-[calc(100dvh-128px)] overflow-y-auto pb-8">
            {panel}
          </div>
        </div>
      )}
