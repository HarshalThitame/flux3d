...
export default function NavbarClient({
  ...
  const whatsappHrefNumber = (whatsappNumber || "+919623023480").replace(
    /[^0-9]/g,
    "",
  );
  ...
  const handleLogout = async () => {
    try {
      ...
      const { useShopWishlistStore } =
        await import("@/stores/shopWishlistStore");
      ...
      >
        ...
        <div className="navbar-left relative z-10 flex min-w-0 items-center gap-4">
          <Link
            href="/"
            prefetch={false}
            className="navbar-logo-link group flex min-h-[56px] items-center px-2"
              ...
              {isProfileOpen && (
                <div className="navbar-profile-menu absolute right-0 top-[calc(100%+0.75rem)] w-[300px] overflow-hidden rounded-2xl p-1.5 backdrop-blur-2xl transition-all duration-200">
                  <div className="border-b border-purple-100/60 p-3.5">
                    ...
                    <p className="truncate text-xs text-purple-500/70">
                      {currentUser.email}
                    </p>
                  </div>
            ...
            >
              ...
              <div className="navbar-mobile-panel absolute right-0 top-0 flex h-[100dvh] w-full max-w-md flex-col overflow-hidden rounded-l-3xl backdrop-blur-2xl">
                ...
                <div className="flex-1 overflow-y-auto overscroll-contain p-6 space-y-6">
                  {currentUser && (
                    <div className="flex items-center gap-3 rounded-2xl bg-[var(--lux-gold-faint)] p-3.5">
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="truncate text-sm font-semibold text-[var(--lux-text-primary)]">
                          {currentUser.name}
                        </p>
                        <p className="truncate text-xs text-[var(--lux-text-muted)]">
                          {currentUser.email}
                        </p>
                      </div>
                    </div>
                  ...
                  {/* Main Links */}
                  <div>
                    <p className="mb-2.5 px-1 font-mono text-[10px] uppercase tracking-wider">
                      Main Menu
                    </p>
                          ...
                          <li key={link.href}>
                            <Link
                              ...
                              className={`flex min-h-[52px] items-center justify-between rounded-xl px-4 py-3 text-base font-medium transition-all duration-200 ${
                                active
                                  ? "bg-[var(--lux-gold-faint)] text-[var(--lux-text-primary)] font-semibold"
                                  : "text-[var(--lux-text-secondary)] hover:bg-[var(--lux-bg-muted)]"
                              }`}
                            >
                              <span>{link.label}</span>
                              {active && (
                                <span className="h-1.5 w-1.5 rounded-full bg-[var(--lux-gold)] shadow-sm" />
                              )}
                            </Link>
                          </li>
                        );
                        ...
                        return (
                          <li key={link.href}>
                            ...
                            >
                              <span>{link.label}</span>
                              {active && (
                                <span className="h-1.5 w-1.5 rounded-full bg-[var(--lux-gold)]" />
                              )}
                            </Link>
                          </li>
                        );
                  ...
                  <div className="pt-2 space-y-3">
                    ...
                    {isAuthPending ? (
                      ...
                      <div className="pt-3 space-y-2">
                        <p className="px-1 font-mono text-[10px] uppercase tracking-wider">
                          Account Options
                        </p>
