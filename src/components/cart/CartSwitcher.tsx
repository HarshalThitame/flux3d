...
export default function CartSwitcher({ variant, quoteCartCount = 0 }: CartSwitcherProps) {
  const shopItems = useShopCartStore((state) => state.items)

  const isQuoteCart = variant === 'quote'
  const OtherCartLabel = isQuoteCart ? '3D Shop Cart' : 'Quote Cart'
  ...
  const borderColor = isQuoteCart ? 'border-violet-200/60' : 'border-emerald-200/60'
  const iconBg = isQuoteCart
    ? 'bg-gradient-to-br from-violet-600 to-indigo-600'
    : 'bg-gradient-to-br from-emerald-600 to-teal-600'

  return (
    <div className={`relative overflow-hidden rounded-2xl border ${borderColor} bg-gradient-to-r ${bgGradient} p-4 sm:p-5`}>
      {/* Decorative gradient blob */}
      <div className={`pointer-events-none absolute -right-8 -top-8 h-32 w-32 rounded-full bg-gradient-to-br ${accentFrom} ${accentVia} ${accentTo} opacity-[0.06] blur-2xl`} />

      <div className="relative flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        {/* Left: info */}
        <div className="flex items-center gap-3.5">
          <div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl ${iconBg} text-white shadow-lg`}>
        ...
        <div className="flex items-center gap-3 sm:gap-4">
          {otherCartCountValue > 0 && (
            <span className={`inline-flex items-center gap-1.5 rounded-full bg-gradient-to-r ${accentFrom} ${accentVia} ${accentTo} px-3 py-1 text-xs font-bold text-white shadow-sm`}>
