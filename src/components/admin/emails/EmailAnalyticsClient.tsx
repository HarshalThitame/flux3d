...
import { MailCheck, Eye, MousePointerClick, AlertTriangle } from "lucide-react";
...
function MetricPill({
  ...
  return (
    <div className="flex items-center gap-3 rounded-2xl border border-gray-200 bg-white p-4">
      <div
        className={`grid h-10 w-10 shrink-0 place-items-center rounded-xl ${color} text-white`}
      >
...
export default function EmailAnalyticsClient() {
  ...
  const fetchAnalytics = async () => {
    ...
    try {
      const res = await fetch(
        `/api/admin/email-analytics?${params.toString()}`,
      );
  ...
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- Fetch analytics when range changes
    fetchAnalytics();
  }, [range]);
