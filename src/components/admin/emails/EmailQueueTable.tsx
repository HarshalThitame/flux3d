...
export default function EmailQueueTable() {
  ...
  const handleCancel = async (id: string) => {
    ...
    setCancellingId(id)
    try {
      const res = await fetch(`/api/admin/email-queue/${id}/cancel`, { method: 'POST' })
      if (res.ok) {
        fetchQueue(page, limit)
      } else {
        const json = await res.json()
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-end gap-3 rounded-2xl border border-gray-200 bg-white p-4">
        <div className="flex flex-col gap-1">
          ...
          <select
            value={statusFilter}
            onChange={(e) => setStatusFilter(e.target.value as EmailQueueStatus | '')}
          ...
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="border-b border-gray-100">
                ...
                <th className="px-5 py-3 text-[10px] font-medium uppercase tracking-[0.15em] text-[#6F7192]">Scheduled</th>
                <th className="px-5 py-3 text-[10px] font-medium uppercase tracking-[0.15em] text-[#6F7192]">Retries</th>
                <th className="px-5 py-3 text-[10px] font-medium uppercase tracking-[0.15em] text-[#6F7192]">Priority</th>
            ...
            <tbody>
              {items.map((item) => (
                <tr key={item.id} className="border-b border-gray-100 transition-colors hover:bg-gray-50">
                  ...
                  <td className="px-5 py-3.5 text-[#0F1B3D]">{item.recipient}</td>
                  <td className="px-5 py-3.5">
                    <span className="text-sm text-[#6F7192]">{item.template_name ?? '—'}</span>
                  </td>
                  ...
                  <td className="px-5 py-3.5 text-xs text-[#6F7192]">{item.priority}</td>
                      ...
                      {item.status !== 'cancelled' && item.status !== 'delivered' && (
                        <>
                          <button
                            ...
                            disabled={retryingId === item.id || item.retry_count >= item.max_retries}
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between rounded-2xl border border-gray-200 bg-white px-4 py-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-[#6F7192]">Page {page} of {totalPages}</span>
            <button
              type="button"
