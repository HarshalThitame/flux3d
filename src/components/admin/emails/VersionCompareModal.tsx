...
export default function VersionCompareModal({
  ...
  const fetchPreview = useCallback(
    async (htmlBody: string) => {
      ...
      return (
        json.html ??
        `<div style="padding:40px;color:red">Preview error: ${json.error ?? "Unknown"}</div>`
      );
    },
    [sampleVars],
  );
  ...
  return (
    ...
    >
      <div
        data-lenis-prevent
        className="flex h-[90vh] w-full max-w-6xl flex-col rounded-2xl border border-gray-200 bg-white shadow-xl"
      >
        {/* Header */}
          ...
          {loading ? (
            ...
            <>
              <div className="flex flex-1 flex-col gap-2 overflow-hidden">
                ...
                <div
                  data-lenis-prevent
                  className="flex-1 overflow-auto rounded-xl bg-gray-100 p-4"
                >
                  <PreviewIframe
                    html={leftHtml}
