...
export default function EmailTemplatesClient({
  ...
  const [testModalTemplate, setTestModalTemplate] =
  ...
  const fetchTemplates = async (p: number, l: number) => {
    ...
    try {
      const res = await fetch(
        `/api/admin/email-templates?${params.toString()}`,
      );
  ...
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- Fetch templates when page changes
  ...
  const openPreview = async (template: EmailTemplateRow) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setPreviewHtml(
          `<p style="color:red">Preview failed: ${json.error ?? "Unknown error"}</p>`,
        );
      }
    } catch {
  ...
  const handleTestSend = async () => {
    ...
    try {
      const res = await fetch(
        `/api/admin/email-templates/${testModalTemplate.id}/test`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col gap-4 rounded-2xl border border-gray-200 bg-white p-4 md:flex-row md:items-end md:justify-between">
        <div className="flex flex-col gap-3 md:flex-row md:items-end">
          ...
          <select
            ...
            onChange={(e) =>
              setCategoryFilter(e.target.value as EmailTemplateCategory | "")
            }
            ...
            >
              ...
              <div className="mt-4 flex flex-wrap gap-2 border-t border-gray-100 pt-4">
                <button
                  type="button"
                  onClick={() =>
                    router.push(`/admin/emails/templates/${t.id}/edit`)
                  }
      ...
      {previewModalTemplate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div
            data-lenis-prevent
            className="flex h-[85vh] w-full max-w-4xl flex-col rounded-2xl border border-gray-200 bg-white shadow-xl"
          >
            <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4">
              <div>
                <h3 className="text-lg font-bold text-[#0F1B3D]">
                  Preview: {previewModalTemplate.name}
                </h3>
                <p className="text-xs text-[#6F7192]">
                  {previewModalTemplate.subject}
                </p>
              </div>
              <button
                type="button"
