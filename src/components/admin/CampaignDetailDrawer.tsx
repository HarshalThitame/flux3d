              ...
              {!loading && !error && campaign && (
                <>
                  ...
                  <div className="rounded-2xl border border-[rgba(109,40,217,0.15)] bg-white p-5">
                    <h3 className="font-semibold text-[#0F1B3D] text-lg">{campaign.name}</h3>
                    <div className="mt-3 grid grid-cols-2 gap-4">
                      ...
                      <div className="flex items-center gap-2 text-sm text-[#6F7192]">
                        ...
                        <span>{new Date(campaign.created_time).toLocaleDateString('en-IN')}</span>
                      </div>
                      <div className="flex items-center gap-2 text-sm text-[#6F7192]">
                        <Layers className="w-4 h-4" />
                        <span>{campaign.adsets?.data?.length ?? 0} ad sets</span>
                      </div>
                    </div>
                  ...
                  <div>
                    ...
                    <AnimatePresence>
                      {showAdsets && (
                        ...
                        >
                          <AdsetsList
                            adsets={(campaign.adsets?.data ?? []) as Array<Record<string, unknown>>}
                          />
                        </motion.div>
                      )}
                    </AnimatePresence>
