...
const EMBEDDING_MODEL =
  process.env.WHATSAPP_EMBEDDING_MODEL?.trim() || "text-embedding-3-small";
...
export async function listWhatsAppKnowledgeChunks(): Promise<
  WhatsAppKnowledgeChunkRecord[]
> {
  const supabase = getServiceClient();
  if (!supabase) {
    return [];
  ...
  const { data, error } = await supabase
    ...
    .select(
      "id, source_key, title, content, tags, priority, active, created_at, updated_at, embedding",
    )
...
export async function getWhatsAppRagContext(
  query: string,
  ...
  if (activeDatabaseChunks.length > 0) {
    const scored = scoreCorpus(activeDatabaseChunks, queryEmbedding).map(
      ({ chunk, score }) => ({
  ...
  console.warn(
    "[rag] whatsapp_knowledge_chunks table is empty — using seed corpus. Run sync-whatsapp-knowledge to populate it.",
  );
  const seedCorpus = await loadSeedCorpus();
  const scored = scoreCorpus(seedCorpus, queryEmbedding).map(
    ({ chunk, score }) => ({
      sourceKey: chunk.sourceKey,
      title: chunk.title,
      score: Number(score.toFixed(3)),
...
export async function syncProductKnowledgeChunks() {
  ...
  const { data: materials, error: materialError } = await supabase
    .from("materials")
    .select(
      "id, name, summary, best_for, key_properties, price_per_gram, sample_photo",
    );
  ...
  if (materialError) {
    ...
    for (const m of materials) {
      ...
      const priceStr = price
        ? `starting at ₹${Number(price).toFixed(2)}`
        : "contact for pricing";
      chunks.push({
...
export async function fetchStructuredData(
  ...
  if (materialError) {
    ...
    const valid = materialData.filter((m: Record<string, unknown>) => {
      const price = m.price_per_gram;
    ...
    materialsData = valid
      ...
      .join("\n");
    materialPrices = valid.map((m: Record<string, unknown>) => ({
      name: m.name,
  ...
  const isOrderQuery =
    intent === "order" || searchTerms.some((k) => /^\d{4,}$/.test(k));
  if (isOrderQuery && supabase) {
    ...
    if (phoneNumber) {
      ...
      const { data: profile } = await supabase
        ...
        .select("id")
        .or(
          `phone_number.eq.${normalizedPhone},phone_number.eq.+${normalizedPhone}`,
        )
        .maybeSingle();
    ...
    if (orderData?.length) {
      orderResults = orderData.map((o: Record<string, unknown>) => ({
      ...
      orderStatusData = orderResults
        .map(
          (o) =>
            `Order #${o.orderNumber} — Status: ${o.status} — Placed: ${o.placedAt ?? "N/A"} — Total: ₹${o.total.toFixed(2)}`,
        )
        .join("\n");
  ...
  if (productError) {
    ...
    const valid = productData.filter(
      (p: Record<string, unknown>) => p.base_price && Number(p.base_price) > 0,
    );
    productsData = valid
      .map((p: Record<string, unknown>) => {
        const formattedPrice = Number(p.base_price).toFixed(2);
        return `Product: ${p.name} | ₹${formattedPrice} | ${String(p.description ?? "").slice(0, 100)}`;
      })
      .join("\n");
    productPrices = valid.map((p: Record<string, unknown>) => ({
      name: p.name,
