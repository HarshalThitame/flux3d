...
export function mapCsvRecord(
  ...
  return {
    ...
    long_description_blocks: record.long_description_blocks
      ? parseJsonCell<unknown>(record.long_description_blocks)
      : undefined,
    ...
    image_alt: record.image_alt
      ? parseJsonCell<Record<string, string>>(record.image_alt)
      : undefined,
    ...
    variants: record.variants
      ? parseJsonCell<ImportVariant[]>(record.variants)
      : undefined,
...
export function productsToCsv(
  ...
  for (const product of products) {
    const join = (value: string[] | null | undefined) =>
      value && value.length > 0 ? value.join(ARRAY_SEPARATOR) : "";
    const values: Record<string, string> = {
      ...
      long_description_blocks:
        ...
        product.long_description_blocks.length > 0
          ? JSON.stringify(product.long_description_blocks)
          : "",
      ...
      image_alt:
        product.image_alt && Object.keys(product.image_alt).length > 0
          ? JSON.stringify(product.image_alt)
          : "",
    ...
    lines.push(
      CSV_COLUMNS.map((column) => escapeCsvCell(values[column] ?? "")).join(
        ",",
      ),
    );
  }
