...
export function isSameState(a: ShopRevision, b: ShopRevision) {
  ...
  return (
    deepStableStringify(a.variant_dimensions ?? []) ===
      deepStableStringify(b.variant_dimensions ?? []) &&
    deepStableStringify(a.variant_option_images ?? []) ===
      deepStableStringify(b.variant_option_images ?? []) &&
    deepStableStringify(a.sku_images ?? []) ===
      deepStableStringify(b.sku_images ?? [])
  );
}
...
const FIELD_LABELS: Record<string, string> = {
  ...
  long_description_blocks: "Luxury description blocks",
...
export function describeChanges(
  ...
  const keys = new Set([
    ...Object.keys(prev.product),
    ...Object.keys(next.product),
  ]);
  ...
  if (variantDelta !== 0) {
    changed.push(
      `${Math.abs(variantDelta)} variant${Math.abs(variantDelta) === 1 ? "" : "s"} ${variantDelta > 0 ? "added" : "removed"}`,
    );
  } else if (
    next.variants.some(
      (variant, index) =>
        deepStableStringify(variant) !==
        deepStableStringify(prev.variants[index]),
    )
  ) {
  ...
  if (skuDelta !== 0) {
    changed.push(
      `${Math.abs(skuDelta)} SKU${Math.abs(skuDelta) === 1 ? "" : "s"} ${skuDelta > 0 ? "added" : "removed"}`,
    );
  } else if (
    next.skus.some(
      (sku, index) =>
        deepStableStringify(sku) !== deepStableStringify(prev.skus[index]),
    )
  ) {
    changed.push("SKU values edited");
  }
  if (
    deepStableStringify(next.variant_dimensions ?? []) !==
    deepStableStringify(prev.variant_dimensions ?? [])
  ) {
    changed.push("Dimensions edited");
  }
  if (
    deepStableStringify(next.variant_option_images ?? []) !==
      deepStableStringify(prev.variant_option_images ?? []) ||
    deepStableStringify(next.sku_images ?? []) !==
      deepStableStringify(prev.sku_images ?? [])
  ) {
