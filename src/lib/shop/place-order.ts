...
export type GuestCheckoutInput = {
  /** Client-generated UUID identifying the anonymous checkout session. */
  sessionId: string
  email: string
  /** SHA-256 hash of the raw guest access token generated by the API route. */
  accessTokenHash: string
}

export type PlaceOrderInput = {
  /** Authenticated user id, or null/undefined for guest checkout. */
  userId?: string | null
  guest?: GuestCheckoutInput | null
...
export async function placeShopOrder(input: PlaceOrderInput): Promise<PlaceOrderResult> {
  ...
  if (!userId && !guest) {
    throw new Error('Either an authenticated user or guest checkout details are required.')
  }
  ...
  if (guest) {
    ...
    const { data: profileMatch } = await supabase
      ...
      .limit(1)
      .maybeSingle()
    claimCandidateUserId = profileMatch && typeof profileMatch.id === 'string' ? profileMatch.id : null
  }
