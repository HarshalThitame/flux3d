...
export async function generateOrderNumber(supabase: ReturnType<typeof createAdminSupabaseClient>, offset = 0) {
  const now = new Date()
...
export function isDuplicateOrderNumberError(error: unknown) {
...
export async function placeShopOrder(input: PlaceOrderInput): Promise<PlaceOrderResult> {
  ...
  for (const rawItem of rawItems) {
    ...
    if (sku.is_available === false || Number(sku.stock_quantity ?? 0) < rawItem.quantity) {
      throw new Error(`Sorry, ${rawItem.skuId} is no longer available in the requested quantity.`)
    }
