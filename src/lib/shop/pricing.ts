...
export type ShopPricingSnapshot = {
  items: Array<{
    ...
    cart_price_rule_id: string | null;
    cart_price_rule_name: string | null;
    cart_price_adjustment: number;
    line_total: number;
  }>;
  subtotal: number;
  discount: number;
...
export type ShopCouponResult = {
  code: string | null;
  discountType:
    "percentage" | "fixed_amount" | "free_shipping" | "buy_x_get_y" | null;
...
export function calculateCouponDiscount(
  ...
  coupon: {
    discount_type:
      string | "percentage" | "fixed_amount" | "free_shipping" | "buy_x_get_y";
...
export function calculateShopTax(
  amount: number,
...
export function buildShopPricingSnapshot(
  ...
  return {
    items: items.map((item) => ({
      ...
      base_unit_price: item.pricingAdjustment?.baseUnitPrice ?? item.unitPrice,
      cart_price_rule_id: item.pricingAdjustment?.ruleId ?? null,
      cart_price_rule_name: item.pricingAdjustment?.ruleName ?? null,
      cart_price_adjustment: item.pricingAdjustment?.amount ?? 0,
