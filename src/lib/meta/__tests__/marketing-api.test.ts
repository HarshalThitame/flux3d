            ...
            data: [
              {
                ...
                base_price: 500,
                shelf_skus: [
                  { sku_code: 'SKU-1', price: 600, variant_image_url: 'https://example.com/vase-red.jpg', is_available: true },
                ],
              },
            ],
            error: null,
...
describe('marketing-api', () => {
  ...
  describe('createCampaign', () => {
    ...
    it('throws on Meta API error', async () => {
      ...
      await expect(
        createCampaign({ name: 'Test', objective: 'OUTCOME_SALES', status: 'PAUSED' })
  ...
  describe('listCampaigns', () => {
    it('returns campaign list from Meta', async () => {
      mockFetch.mockResolvedValue(makeJsonResponse({
        data: [
          { id: 'camp-1', name: 'Campaign 1', objective: 'OUTCOME_SALES', status: 'ACTIVE', effective_status: 'ACTIVE', created_time: '2024-01-01', updated_time: '2024-01-02' },
        ],
      }))
  ...
  describe('createCarouselCreative', () => {
    it('creates a carousel creative with cards', async () => {
      ...
      const cards: CarouselCard[] = [
        { link: 'https://flux3d.in/1', picture: 'https://img/1.jpg', name: 'Product 1', description: '₹100' },
      ]
  ...
  describe('updateCampaignStatus', () => {
    it('posts status update to Meta', async () => {
      ...
      expect(call[1]).toMatchObject({ method: 'POST', body: JSON.stringify({ status: 'ACTIVE' }) })
    })
  })
  ...
  describe('updateCampaignBudget', () => {
    it('posts budget update to Meta', async () => {
      ...
      const call = mockFetch.mock.calls[0] as unknown[]
      expect(call[1]).toMatchObject({ method: 'POST', body: JSON.stringify({ daily_budget: 20000 }) })
    })
  })
  ...
  describe('updateCampaignName', () => {
    it('posts name update to Meta', async () => {
      ...
      const call = mockFetch.mock.calls[0] as unknown[]
      expect(call[1]).toMatchObject({ method: 'POST', body: JSON.stringify({ name: 'New Name' }) })
    })
  })
  ...
  describe('getCampaignInsights', () => {
    it('returns insights for given campaign IDs', async () => {
      mockFetch.mockResolvedValue(makeJsonResponse({
        data: [
          { campaign_id: 'camp-1', campaign_name: 'Test', spend: '100', impressions: '1000', clicks: '50', ctr: '5', cpc: '2', conversions: '2', cost_per_conversion: '50' },
        ],
      }))
  ...
  describe('fetchNewArrivalProductsForAd', () => {
    it('returns product cards with SKUs', async () => {
      ...
      const result = await fetchNewArrivalProductsForAd(supabase, 'Home Decor', 10)
  ...
  describe('retry behavior', () => {
    ...
    it('retries on 429 and succeeds on second attempt', async () => {
      mockFetch
        ...
        .mockResolvedValueOnce(makeJsonResponse({ data: [{ id: 'camp-1', name: 'Test', objective: 'SALES', status: 'ACTIVE', effective_status: 'ACTIVE', created_time: '2024-01-01', updated_time: '2024-01-02' }] }))
  ...
  describe('error extraction', () => {
    it('extracts error_user_msg when present', async () => {
      mockFetch.mockResolvedValue({
        ok: false,
        status: 400,
        json: () => Promise.resolve({ error: { error_user_msg: 'Please use a valid page ID.' } }),
      } as Response)

      await expect(listCampaigns()).rejects.toThrow('Please use a valid page ID.')
    })
