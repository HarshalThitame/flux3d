...
// Per-type size limits enforced by WhatsApp Cloud API
export const WHATSAPP_SIZE_LIMITS: Record<string, number> = {
...
/**
 * Generate a 200×200 WebP thumbnail from an image buffer.
 ...
 */
export function extractWhatsAppMediaStoragePath(
...
export async function downloadAndStoreWhatsAppMedia(
  ...
  try {
    ...
    if (!metaRes.ok) {
      console.error(
        "[whatsapp-media] Failed to fetch media metadata:",
        await metaRes.text(),
      );
      return null;
    }
    ...
    const mimeType =
      metaData.mime_type || mimeTypeHint || "application/octet-stream";
    ...
    if (!binaryRes.ok) {
      console.error(
        "[whatsapp-media] Failed to download binary media content:",
        binaryRes.status,
      );
      return null;
    }
    ...
    if (uploadErr) {
      console.error(
        "[whatsapp-media] Supabase storage upload failed:",
        uploadErr,
      );
      return null;
    }
    ...
    if (mediaType === "image") {
      ...
      if (thumbBuffer) {
        const thumbPath = `inbound/thumb_${Date.now()}_${filename.replace(/[^a-zA-Z0-9._-]/g, "_")}.webp`;
...
export async function sendWhatsAppMediaMessage(
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `WhatsApp send media failed: ${res.status} ${errorText.slice(0, 300)}`,
    );
  }
