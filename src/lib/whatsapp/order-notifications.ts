...
export async function enqueueOrderNotification(
  ...
  if (defError) {
    console.error(
      `[whatsapp-notify] Error fetching notification config for ${newStatus}:`,
      defError,
    );
    return;
  }
  ...
  const { data: order, error: orderError } = await supabase
    .from("orders")
    .select(
      "id, group_id, order_number, phone, full_name, total_price, user_id, updated_at",
    )
    .eq("id", orderId)
    .maybeSingle();
  ...
  if (orderError || !order) {
    console.error(
      `[whatsapp-notify] Error fetching order ${orderId}:`,
      orderError,
    );
    return;
  }
  ...
  if (!order.phone) {
    console.log(
      `[whatsapp-notify] Skip sending for order ${orderId} - no phone number`,
    );
    return;
  }
  ...
  if (order.user_id) {
    ...
    const isOptedOut =
      profile &&
      (profile as Record<string, unknown>).whatsapp_opted_out === true;
    if (isOptedOut) {
      console.log(
        `[whatsapp-notify] Skip sending for order ${orderId} - user opted out`,
      );
      return;
    }
  }
  ...
  const cooldownWindow = new Date(
    Date.now() - notificationDef.cooldown_minutes * 60000,
  ).toISOString();
  ...
  if (jobsError) {
    console.error(
      `[whatsapp-notify] Error checking job idempotency for ${orderId}:`,
      jobsError,
    );
    return;
  }
  ...
  if (existingJobs && existingJobs.length > 0) {
    const hasSuccessfulOrPending = existingJobs.some(
      (job) => job.status === "sent" || job.status === "pending",
    );
    if (hasSuccessfulOrPending) {
      console.log(
        `[whatsapp-notify] Idempotency guard: notification already sent or pending for order ${orderId} status ${newStatus}`,
      );
      return;
    }
    // If it's a recent failure, the cooldown guard still prevents it because it's in existingJobs.
    console.log(
      `[whatsapp-notify] Cooldown guard: notification attempted recently for order ${orderId} status ${newStatus}`,
    );
    return;
  }
  ...
  if (insertError) {
    console.error(
      `[whatsapp-notify] Failed to enqueue job for order ${orderId}:`,
      insertError,
    );
  }
}
