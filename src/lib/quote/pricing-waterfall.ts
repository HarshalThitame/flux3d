...
export function calculateQuotePricing(
  ...
  const partWeightGramsPerUnit =
    estimatedPartVolumeCm3 * material.density * material.multiplier;
  const supportWeightGramsPerUnit = config.supports
    ? supportVolumeCm3 * material.density * material.multiplier
    : 0;
  const materialWeightGramsPerUnit =
  ...
  const supportCost = config.supports
  ...
  const estimatedMinutesPerUnit =
    basePrintTimeMinutesPerUnit * layerHeightMultiplier;
