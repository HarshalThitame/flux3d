...
export function calculateDeliveryChargeFromSettings(
  ...
  settings: Pick<
    BusinessSettings,
    "deliveryChargeThreshold" | "defaultDeliveryCharge"
  >,
) {
...
export function getHighestCartDiscountTier(
  ...
  return (
    [...tiers]
      ...
      .sort(
        (left, right) =>
          positiveNumber(right.minCartValue) -
          positiveNumber(left.minCartValue),
      )[0] ?? null
  );
}
...
export function calculatePromotionDiscount(
  ...
  const maxDiscount =
    promotion.maxDiscount == null
      ? null
      : positiveNumber(promotion.maxDiscount);
  ...
  if (promotion.discountType === "percentage") {
    ...
    return roundMoney(
      maxDiscount == null || maxDiscount <= 0
        ? calculated
        : Math.min(calculated, maxDiscount),
    );
  }
...
export function calculatePostProcessingCharge(
  ...
  return roundMoney(
    baseAmount * multiplier * positiveNumber(difficultyFactor, 1),
  );
}
...
export function calculatePricingWaterfall(
  ...
  const subtotal = roundMoney(
    materialCost + machineCost + postProcessingCharges,
  );
  ...
  const cartDiscountAmount = roundMoney(
    totalPrice * (cartDiscountPercent / 100),
  );
  const afterCart = roundMoney(Math.max(0, totalPrice - cartDiscountAmount));
  const couponDiscountAmount = calculatePromotionDiscount(
    afterCart,
    input.coupon,
  );
  const afterCoupon = roundMoney(Math.max(0, afterCart - couponDiscountAmount));
  const offerDiscountAmount = calculatePromotionDiscount(
    afterCoupon,
    input.offer,
  );
  ...
  const minimumOrderValue = Math.max(
    0,
    positiveNumber(input.minimumOrderValue, 0),
  );
  const priceBeforeMinimum = finalPrice;
  const priceAfterMinimum =
    minimumOrderValue > 0 && finalPrice > 0 && finalPrice < minimumOrderValue
  ...
  const discount = roundMoney(
    cartDiscountAmount + couponDiscountAmount + offerDiscountAmount,
  );
...
export function calculateQuotePricing(
  ...
  const layerHeight =
    layerHeightOptions.find((option) => option.value === config.layerHeight) ??
    layerHeightOptions[0];
  ...
  const hasSlicer = Boolean(model.slicerResult);
  ...
  const markupMultiplier =
    1 + positiveNumber(settings.materialMarkupPercent, 0) / 100;

  // Per-color weights (for multi-color AMS pricing)
  const weightsPerColor = hasSlicer
    ? model.slicerResult!.weightsPerColor
    : [materialWeightGramsTotal];

  // AMS Surcharge calculation
  const numColors = config.amsColorCount ?? 1;
  const amsColorChangeSurcharge =
    hasSlicer && numColors > 1
      ? (numColors - 1) * positiveNumber(settings.amsColorChangeSurcharge, 30)
      : 0;

  const materialCostPerUnit =
    materialWeightGramsPerUnit * material.pricePerGram * markupMultiplier;
  // AMS surcharge added to total material cost
  const materialCost = materialCostPerUnit * quantity + amsColorChangeSurcharge;

  const supportCost =
    config.supports && !hasSlicer
      ? supportWeightGramsTotal * material.pricePerGram
      : 0;

  const printSpeedGramsPerHour = Math.max(
    0.01,
    positiveNumber(settings.printSpeedGramsPerHour, 40),
  );
  const basePrintTimeMinutesPerUnit =
    (materialWeightGramsPerUnit / printSpeedGramsPerHour) * 60;
  const layerHeightMultiplier = layerHeight.multiplier;

  // Time: slicer wins over formula
  const estimatedMinutes = hasSlicer
    ? model.slicerResult!.estimatedMinutes
    : basePrintTimeMinutesPerUnit * layerHeightMultiplier * quantity;

  const estimatedMinutesPerUnit = estimatedMinutes / quantity;
  const estimatedHours = estimatedMinutes / 60;

  const machineRatePerHour = material.machineRate;
  const machineCostPerUnit =
