...
describe("getPostProcessingCharge", () => {
  it("uses FALLBACK_SETTINGS multipliers by default", () => {
    ...
    const expectedMultiplier =
      FALLBACK_SETTINGS.postProcessingMultipliers["sanded"];
  ...
  it("respects custom multipliers", () => {
    const charge = getPostProcessingCharge("sanded", 100, 1.0, {
      none: 0,
      sanded: 0.5,
      "sanded-painted": 1.0,
    });
...
describe("calculateInstantQuote", () => {
  ...
  it("returns null when material is not found", async () => {
    ...
    const config = {
      ...
      layerHeight: 0.2,
      quantity: 1,
      postProcessingLevel: "none" as const,
      supports: false,
    };
  ...
  it("uses the layerHeight multiplier from layerHeightOptions, not 0.2 / value", async () => {
    ...
    const standard = calculateInstantQuote(
      model,
      { ...createConfig(0.2) },
      [material],
      settings,
    );
    const high = calculateInstantQuote(
      model,
      { ...createConfig(0.12) },
      [material],
      settings,
    ...
    if (standard && high) {
      ...
      expect(high.estimatedMinutesPerUnit).toBeCloseTo(
        standard.estimatedMinutesPerUnit * 1.3,
        4,
      );
      // Legacy behavior (0.2 / 0.12 = 1.6667) must NOT be used
      expect(high.estimatedMinutesPerUnit).toBeLessThan(
        standard.estimatedMinutesPerUnit * 1.67,
      );
    }
