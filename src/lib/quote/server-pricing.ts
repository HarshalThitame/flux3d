...
export function toParsedModel(metadata: ModelMetadata): ParsedModel {
  return {
    ...
    surfaceAreaMm2: metadata.surfaceAreaMm2 ?? 0,
    supportVolumeMm3: metadata.supportVolumeMm3 ?? 0,
...
export async function fetchMaterialForQuote(
  materialId: string,
): Promise<QuoteMaterial | null> {
  ...
  const colors = Array.isArray(row.colors)
    ? (row.colors as Array<{ name?: string; hex?: string }>).map((c) => ({
        name: c.name ?? "Default",
      }))
    : [];
  ...
  return {
    ...
    recommendedFor:
      typeof row.recommended_for === "string" ? row.recommended_for : "",
    properties: (() => {
      const props =
    ...
    difficultyLevel:
      (row.difficulty_level as QuoteMaterial["difficultyLevel"]) ?? "Easy",
    heatResistance:
      (row.heat_resistance as QuoteMaterial["heatResistance"]) ?? "Low",
    strengthRating:
      (row.strength_rating as QuoteMaterial["strengthRating"]) ?? "Medium",
    finishQuality:
      (row.finish_quality as QuoteMaterial["finishQuality"]) ?? "Good",
...
export async function calculateServerQuotePricing(
  ...
  if (!material) {
    throw new Error(
      "Selected material is not available. Please choose a different material.",
    );
  }
