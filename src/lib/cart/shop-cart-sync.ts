...
function sameEntry(
  line: ServerCartLine,
  item: Pick<ShopCartItem, "skuId" | "customizationText">,
) {
  return (
    line.sku_id === item.skuId &&
    String(line.payload?.customizationText ?? "").trim() ===
      item.customizationText.trim()
  );
}

function lineToShopItem(
  line: ServerCartLine,
  live?: LiveSkuSnapshot,
): ShopCartItem {
  const payload = (line.payload ?? {}) as Record<string, unknown>;
  const variantCombination = (payload.variantCombination ?? {}) as Record<
    string,
    string | boolean
  >;
  const stockSource = live
    ? Math.max(1, live.stockQuantity)
    : Number(payload.maxStock ?? 1);
  ...
  return {
    ...
    catalogRetailerId: payload.catalogRetailerId
      ? String(payload.catalogRetailerId)
      : undefined,
    variantCombination,
    variantLabel: String(
      payload.variantLabel || buildVariantLabel(variantCombination),
    ),
...
function shopItemToLine(userId: string, item: ShopCartItem): NewServerCartLine {
  return {
    ...
    weight_grams: item.weightGrams
      ? Math.round(item.weightGrams * item.quantity)
      : null,
    payload: {
      ...
      catalogRetailerId: item.catalogRetailerId,
...
function applyHydratedItems(nextItems: ShopCartItem[]) {
  ...
  const previousByLineId = new Map(
    state.items.map((item) => [item.cartItemId, item]),
  );
  ...
  useShopCartStore.setState({
    items: nextItems,
    priceChangedItemIds: Array.from(
      new Set([...state.priceChangedItemIds, ...changedIds]),
    ),
  });
}
...
export async function adoptServerShopCart(userId: string) {
  ...
  try {
    ...
    const localOnlyItems = useShopCartStore
      .getState()
      .items.filter((item) => item.localOnly);
...
export function mirrorShopQuantity(
  cartItemId: string,
  quantity: number,
  unitPrice: number,
) {
...
export function mirrorShopClear() {
  void clearServerCart("shop").catch((error) =>
    console.warn("[shop-cart] Clear sync failed", error),
  );
}
